...
<!--        </a-form-model-item>-->
        ...
        <a-form-model-item label="可领取数">
          <a-radio-group v-model="formData.num_type">
            <a-radio value="1">
              增加
            </a-radio>
            <a-radio value="2">
          ...
          <a-input type="number" style="width: 150px;margin-left: 50px;" v-model="formData.num" />
          <br><span>备注：剩余可领取数{{formData.send_num - formData.get_num}}</span>
        </a-form-model-item>
...
export default {
  ...
  created() {
    this.$set(this.formData, 'num', 0);
    this.$set(this.formData, 'num_type', "1");
  ...
  methods: {
    ...
    handleOk() {
      ...
      if (this.formData.num !== 0) {
        if (this.formData.num < 0) {
          this.$message.error("设置可领取数不可小于0！");
          return false;
        }
        if (this.formData.num_type === 2 && (this.formData.send_num - this.formData.get_num) < this.formData.num) {
          this.$message.error("剩余可领取数不足！");
          return false;
        }
      }
      this.request(scanPlatformApi.editData, this.formData).then((res) => {
