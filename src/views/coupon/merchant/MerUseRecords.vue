                    ...
                    <a-col>
                        <a-form-item label="业务">
                         ...
                         <a-form-item label="店铺">
                            <a-select
                                    label-in-value
                                    :default-value="{ key:queryParam.store_id }"
                                    style="width: 200px"
                                    @change="handleStoreIdChange"
                            >
                                <a-select-option value="0">全部门店</a-select-option>
                                <a-select-option v-for="(item,index) in shopData" :value="item.store_id">{{item.name}}}</a-select-option>
                            </a-select>
                        </a-form-item>
...
<script>
    ...
    export default {
        ...
        data() {
            ...
            return {
                search_data: [],
                shopData:[{'store_id':429,'name':'张涛专属测试店铺'},{'store_id':430,'name':'张涛店铺'}],
        ...
        methods: {
            ...
            handleStoreIdChange(value){
                this.queryParam.store_id = value.key;
            },
            ...
            getStoreLists(){
                    this.request(couponPlatformApi.getStoreList,this.queryParam)
                    .then((res) => {
                        this.shopData = res
                    })
            },
            getSearchHotList() {
                this.request(couponPlatformApi.merCouponUseRecords,this.queryParam)
                    .then((res) => {
                        console.log('res', res)
