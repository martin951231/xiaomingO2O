...
export default {
  ...
  watch: {
    '$route.query.store_id'(val) {
      ...
      if ('/merchant/merchant.mall/limitedList' == this.$route.path && val) {
        this.store_id = val
  ...
  methods: {
    ...
    getDataList(params) {
      ...
      this.request(mallMerchantApi.getLimitedList, params).then((res) => {
    ...
    // 新建活动
    createActivity() {
      this.$router.push({ path: '/merchant/merchant.mall/editLimited', query: { store_id: this.store_id } })
    },
    // 编辑活动
    editActivity(id, status) {
      if (status == 2) {
        this.$router.push({ path: '/merchant/merchant.mall/EditLimitedLook', query: { store_id: this.store_id, id } })
      } else {
        this.$router.push({ path: '/merchant/merchant.mall/EditLimited', query: { store_id: this.store_id, id } })
      }
    },
    // 失效
    changeActivityState(id) {
      this.$confirm({
        ...
        centered: true,
        onOk: () => {
          this.request(mallMerchantApi.limitedChangeState, { id }).then((res) => {
    ...
    removeActivity(id) {
      this.$confirm({
        ...
        centered: true,
        onOk: () => {
          this.request(mallMerchantApi.removeLimited, { id }).then((res) => {
            this.$message.success('操作成功！')
            this.submitForm()
          })
        },
