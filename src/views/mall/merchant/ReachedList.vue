...
export default {
  ...
  watch: {
    '$route.query.store_id'(val) {
      ...
      if ('/merchant/merchant.mall/reachedList' == this.$route.path && val) {
        this.store_id = val
  ...
  activated() {
    //   添加活动后更新订单列表
    let reachedEdit = sessionStorage.getItem('ReachedEdit') || ''
    if (reachedEdit && reachedEdit == 1) {
      this.store_id = this.$route.query.store_id
      this.$set(this.pagination, 'current', 1)
      this.$set(this.pagination, 'pageSize', 10)
      this.$set(this.pagination, 'total', 0)
      this.getDataList({ store_id: this.store_id })
      sessionStorage.removeItem('ReachedEdit')
    }
  },
  methods: {
    moment,
    ...
    getDataList(params) {
      ...
      this.request(mallMerchantApi.getReachedList, params).then((res) => {
    ...
    // 新建活动
    createActivity() {
      this.$router.push({ path: '/merchant/merchant.mall/editReached', query: { store_id: this.store_id } })
    },
    // 编辑活动
    editActivity(id, status) {
      if (status == 2) {
        this.$router.push({
          path: '/merchant/merchant.mall/editReachedLook',
          query: { store_id: this.store_id, id: id },
        })
      } else {
        this.$router.push({ path: '/merchant/merchant.mall/editReached', query: { store_id: this.store_id, id: id } })
      }
    },
    // 编辑活动状态
    changeActivityState(id) {
      this.$confirm({
        ...
        centered: true,
        onOk: () => {
          this.request(mallMerchantApi.reachedChangeState, { id }).then((res) => {
    ...
    removeActivity(id) {
      this.$confirm({
        ...
        centered: true,
        onOk: () => {
          this.request(mallMerchantApi.reachedDel, { id }).then((res) => {
            this.$message.success('操作成功！')
            this.submitForm()
          })
        },
