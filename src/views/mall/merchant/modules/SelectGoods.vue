                ...
                <template v-for="sMenu of menu.children">
                  ...
                  <template v-if="sMenu.children && sMenu.children.length">
                    <a-sub-menu :key="sMenu.id" :title="sMenu.name">
                      <a-menu-item v-for="tMenu of sMenu.children" :key="tMenu.id">{{ tMenu.name }}</a-menu-item>
                    </a-sub-menu>
                  </template>
                  <!-- 二级菜单 -->
                  <template v-else>
        ...
        <div class="bottom">
          ...
          >
            ...
            <span slot="price" slot-scope="text, record">
              <span v-if="record.goods_type == 'sku'" key="sku_goods"
            ...
            >
              ...
              <span class="flex align-center mb-20 cr-99" v-for="item of record.sku_info" :key="item.sku_id">
                ...
                <span class="flex-1">{{ item.stock_num == -1 ? '无限量' :  item.stock_num}}</span>
              </span>
              <!-- </template> -->
              <!-- <template v-else>
                <a-radio-group :default-value="record.defaultValue" @change="onRadioChange(record, $event)">
                  <a-radio
                    class="mb-20 cr-99"
                    v-for="item of record.sku_info"
                    :key="item.sku_id"
                    :disabled="item.can_be_choose == 0"
                    :value="item.sku_id"
                    style="display: block !important; margin-bottom: 20px; color: #999; width: 100% !important"
                  >
                    <span class="flex align-center inline-block">
                      <span class="ml-20 mr-20 inline-block" style="width: 245px">{{ item.sku_str || '--' }}</span>
                      <span class="inline-block" style="width: 200px">￥{{ item.price }}</span>
                      <span class="flex-1 inline-block">{{ item.stock_num }}</span>
                    </span>
                  </a-radio>
                </a-radio-group>
              </template> -->
...
export default {
  ...
  methods: {
    ...
    // 获取左侧分列列表
    getSortList() {
      this.request(mallMerchantApi.getMerchantSort, { store_id: this.storeId }).then((res) => {
    ...
    getGoodsList() {
      ...
      this.request(mallMerchantApi.getMallGoods, params).then((res) => {
        if (res.list && res.list.length) {
    ...
    handleOk() {
      ...
      if (this.isSku) {
        if (sList.length) {
          ...
          this.$emit('submit', { ids: '', goods: list })
          this.handleCancel()
        } else {
    ...
    isSkuGoods() {
      if (
        this.source == 'periodic' ||
        this.source == 'reached' ||
        this.source == 'shipping' ||
        this.source == 'minus_discount' ||
        this.source == 'give' ||
        this.source == 'spec' ||
        this.source == 'prepare' ||
        this.source == 'group' ||
        ...
        // spu goods （spec是做特殊处理的比如选择满赠的商品需要选规格）
        this.isSku = false
      } else {
        // sku goods: bargain full-give limited group prepare
        this.isSku = true
