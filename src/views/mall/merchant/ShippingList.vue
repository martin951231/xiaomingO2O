  ...
  <div class="mt-10 mb-20 pt-20 pl-20 pr-20 pb-20 bg-ff br-10">
    ...
    <a-form-model layout="inline" :model="searchForm">
      <a-form-model-item label="店铺名称">
        <a-input v-model="searchForm.name" placeholder="请输入店铺名称" />
      </a-form-model-item>
...
export default {
  ...
  watch: {
    '$route.query.store_id'(val) {
      ...
      if ('/merchant/merchant.mall/shippingList' == this.$route.path && val) {
        this.store_id = val
  ...
  activated() {
    //   添加活动后更新订单列表
    let ShippingEdit = sessionStorage.getItem('shippingEdit') || ''
    if (ShippingEdit && ShippingEdit == 1) {
      this.store_id = this.$route.query.store_id
      this.$set(this.pagination, 'current', 1)
      this.$set(this.pagination, 'pageSize', 10)
      this.$set(this.pagination, 'total', 0)
      this.getDataList({ store_id: this.store_id })
      sessionStorage.removeItem('shippingEdit')
    }
  },
  methods: {
    moment,
    ...
    getDataList(params) {
      ...
      this.request(mallMerchantApi.getShippingList, params).then((res) => {
    ...
    createActivity() {
      this.$router.push({
        ...
        query: { store_id: this.store_id, type: 'add' },
      })
    },
    // 编辑活动
    editActivity(id, status) {
      if (status == 2) {
        this.$router.push({
          path: '/merchant/merchant.mall/EditShippingLook',
          query: { store_id: this.store_id, id: id },
        })
      } else {
        this.$router.push({
          path: '/merchant/merchant.mall/EditShipping',
          query: { store_id: this.store_id, id, type: 'edit' },
        })
      }
    },
    // 失效
    changeActivityState(id) {
      this.$confirm({
        ...
        centered: true,
        onOk: () => {
          this.request(mallMerchantApi.changeState, { id }).then((res) => {
    ...
    removeActivity(id) {
      this.$confirm({
        ...
        centered: true,
        onOk: () => {
          this.request(mallMerchantApi.removeShipping, { id }).then((res) => {
            this.$message.success('操作成功！')
            this.submitForm()
          })
        },
