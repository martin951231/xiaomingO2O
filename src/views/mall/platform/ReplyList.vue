  ...
  <div class="mt-10 mb-20 pt-20 pl-20 pr-20 pb-20 bg-ff br-10">
    ...
    <a-form-model layout="inline" :model="searchForm">
      <a-form-model-item label="搜索">
        ...
        <a-input v-model="searchForm.content" placeholder="请输入商品名称、店铺名称" style="width:215px;"/>
      </a-form-model-item>
<!--      <a-form-model-item label="全部店铺">-->
<!--        <a-select-->
<!--          v-model="searchForm.type"-->
<!--          style="width:115px;"-->
<!--        >-->
...
<!--      </a-form-model-item>-->
      ...
      <a-form-model-item>
        ...
        <a-button @click="exportExcel()" class="ml-20"> 导出</a-button>
      </a-form-model-item>
    </a-form-model>
    <!-- 数据列表 -->
    <a-table class="mt-20" rowKey="rpl_id" :columns="columns" :data-source="dataList" :pagination="pagination">
      ...
      <span slot="is_show" slot-scope="text,record">
         <a-switch
                ...
                @change="displaySwitch(record.rpl_id)"
        />
      </span>
      <!-- 操作栏 -->
...
export default {
  ...
  methods: {
    ...
    getDataList(param) {
      ...
      this.request(mallPlatformApi.getReplyList, params).then((res) => {
    ...
    removeComment(id) {
      this.$confirm({
        ...
        centered: true,
        onOk: () => {
          this.request(mallPlatformApi.delReply, { rpl_id: id }).then((res) => {
    ...
    { 
       this.request(mallPlatformApi.isShowReply, { rpl_id: rpl_id }).then((res) => {
         this.$message.success('操作成功！')
           this.getDataList({is_search: false});
       })
    },
    ...
    exportExcel(){
      ...
      params.pageSize = this.pagination.pageSize
      params.is_export = 1
      this.request(mallPlatformApi.exportReplyList, params).then((res) => {
