...
<script>
  ...
  export default {
    name: 'ActivityGroupRecommend',
    components: {
      SetRecommend,
    },
    data() {
      return {
        activeKey:'getGroupRecommendList',
        searchForm: {
          merList: [],
          storeList: [],
          keyword: '',
        ...
        columnsGoods: [
          ...
          },
          {
            title: '活动状态',
            dataIndex: 'status',
            scopedSlots: { customRender: 'status_des' }
          },
          {
            title: '原价',
            dataIndex: 'price',
    ...
    methods: {
      ...
      getGroupRecommendList() {
        this.request(mallPlatformApi.getGroupRecommendList, this.queryParam).then((res) => {
            console.log(res.list)
            this.searchHotList = res.list;
            this.pagination.total = res.count;
          }
