            ...
            <div style="margin-top: 20px;margin-left: 50px;" class="editword" required>
                ...
                <div v-if="postdata.xtype===0 || postdata.xtype==='0'">
                    <a-form-item label="链接" required>
                        <div v-for="(item, index) in wordurllist" :key="index">
                            <div>
                                <a-input placeholder="请选择链接" v-model.trim="wordurllist[index].jumpurl" style="width: 300px"/>
<!--                                <a-input placeholder="请选择链接" v-decorator="[`wordurllist[${index}].jumpurl`, {initialValue:wordurllist[index].jumpurl}]" style="width: 300px" disabled/>-->
                                <a-button class="ant-form-text" @click="setLinkBases(index)">
                                    功能库选择
                                </a-button>
...
<script>
    ...
    import FunctionLibrary from '@/views/community/village/framework/FunctionLibrary.vue';
    ...
    export default {
        ...
        methods: {
            ...
            getOneHotWord(){
                ...
                this.request(hotWordManageApi.getOneHouseHotWord, postArr)
                    .then((res) => {
            ...
            handleSubmit() {
                ...
                console.log(postArr)
                that.request(hotWordManageApi.saveHotWordData, postArr).then((res) => {
            ...
            setLinkBases(index) {
                this.$LinkBases({
                    ...
                    handleOkBtn: (res) => {
                        ...
                        this.url = res.url
                        console.log('123123', res.url)
                        this.$nextTick(() => {//链接库动态赋值
                            this.wordurllist[index].jumpurl=this.url
                            // this.form.setFieldsValue({
                            //     jumpurl[index]: this.url,
                            // })
                        });
                    },
                    // handleCancelBtn: () => {
            ...
            beforeUpload(file) {
                ...
                if (!isJpgOrPng) {
                    this.$message.error('您只能上传jpg,png,jpeg,gif文件!');
                }
