<template>
    <div>
        <a-button type="primary" icon="plus" @click="$refs.createModal.add()" style="margin: 15px 20px 15px auto"
            >新建搜索词</a-button
        >
        <a-button class="icon_btn" @click="delete_selected_word()">删除</a-button>
        <a-table
            :columns="columns"
            :data-source="searchHotList"
        ...
        >
            ...
            <span slot="action" slot-scope="text, record">
                <a @click="$refs.createModal.edit(record.pigcms_id)">编辑</a>
                <a-divider type="vertical" />
                <a-popconfirm
                    ...
                    ok-text="Yes"
                    cancel-text="No"
                    @confirm="deleteConfirm(record.pigcms_id)"
                    @cancel="cancel"
                >
                    <a href="#">删除</a>
                </a-popconfirm>
        ...
        <create-search-hot-words ref="createModal" :sourceInfo="sourceInfo" @ok="handleOk" />
    </div>
</template>
    ...
    methods: {
        delete_selected_word() {
            if (this.selectedRowKeys.length < 1) {
                ...
                this.request(hotwordsPlatformApi.delWords, {
        ...
        getSearchHotList() {
            this.$set(this.queryParam, 'source', this.sourceInfo.source)
            this.$set(this.queryParam, 'source_id', this.sourceInfo.source_id)
            this.request(hotwordsPlatformApi.getHotWordsList, this.queryParam).then((res) => {
                console.log('res', res)
        ...
        save(key) {
            ...
            console.log(target)
            this.request(hotwordsPlatformApi.getHotWordsEditSort, { id: target.pigcms_id, sort: target.sort }).then(
                (res) => {
                    this.getSearchHotList()
                }
            )
