<template>
  <div>
    <!-- 组件描述 -->
    <componentDesc :content="desc"></componentDesc>
    <div v-if="formDataDecorate" class="content">
      <a-form-model :model="formDataDecorate" :label-col="labelCol" :wrapper-col="wrapperCol" :labelAlign="'left'">
        <a-form-model-item :label="L('空白高度')">
          <a-row type="flex">
            <a-col :span="17">
              <a-slider v-model="formDataDecorate.blank_height" :max="100" :min="10" />
            </a-col>
            <a-col :span="6" :offset="1">
              <a-input-number v-model="formDataDecorate.blank_height" :min="10" :max="100"></a-input-number>
            </a-col>
          </a-row>
        </a-form-model-item>
      </a-form-model>
    </div>
  </div>
</template>

<script>
import componentDesc from '@/views/common/customPage/modules/common/componentDesc'
import IconFont from '@/utils/icon'
export default {
  components: {
    componentDesc,
    IconFont
  },
  props: {
    formContent: {
      type: [String, Object],
      default: ''
    }
  },
  data() {
    return {
      desc: {
        title: this.L('辅助空白')
      },
      labelCol: { span: 4 },
      wrapperCol: { span: 20 },
      formDataDecorate: ''
    }
  },
  watch: {
    formContent: {
      deep: true,
      handler(newVal, oldVal) {
        if (newVal) {
          this.formDataDecorate = {}
          for (let k in newVal) {
            this.$set(this.formDataDecorate, k, newVal[k])
          }
        } else {
          this.formDataDecorate = ''
        }
      }
    },
    formDataDecorate: {
      deep: true,
      handler(val) {
        this.$emit('updatePageInfo', val)
      }
    }
  },
  mounted() {
    if (this.formContent) {
      this.formDataDecorate = {}
      for (let k in this.formContent) {
        this.$set(this.formDataDecorate, k, this.formContent[k])
      }
    }
  }
}
</script>

<style scoped>
.content {
  padding: 24px;
  width: 100%;
  box-sizing: border-box;
}
</style>
