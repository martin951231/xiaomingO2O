    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item v-if="$store.getters.config" :label="'绑定'+$store.getters.config.house_name" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select @change="get_single_list" v-decorator="['village_id', {initialValue:village_id,rules: [{required: true,  message: '请选择'+$store.getters.config.house_name}]}]" :placeholder="'请选择'+$store.getters.config.house_name">
            <a-select-option v-for="list in village_list" :key="list.village_id">{{ list.village_name }}</a-select-option>
          </a-select>
        </a-form-item>

        <a-form-item v-if="$store.getters.config" :label="'绑定'+$store.getters.config.single_name" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select v-decorator="['single_id', {initialValue:single_id,rules: [{required: true,  message: '请选择'+$store.getters.config.single_name}]}]" :placeholder="'请选择'+$store.getters.config.single_name"  mode="multiple" >
            <a-select-option v-for="list in single_list" :key="list.single_id">{{ list.single_name }}</a-select-option>
          </a-select>
        </a-form-item>
        ...
        methods: {
            ...
            edit(id){
                ...
                this.request(streetCommunityApi.getGridRangeInfo,{
                    ...
                    this.request(streetCommunityApi.getSingleList, {
            ...
            //获取小区下楼栋列表
            get_single_list(key,val){
                this.request(streetCommunityApi.getSingleList, {
                    village_id : val.data.key
                }).then((res) => {
                    this.single_list = res;
                    console.log(res)
                })
            ...
            handleSubmit () {
                ...
                validateFields((errors, values) => {
                    if(values.grid_member_id == null || values.polygon_name == null || values.village_id == null || values.single_id == null){
                        ...
                        if(this.is_edit == 0){
                            this.request(streetCommunityApi.addGridRange, {
                                zoom:this.zoom,lng:this.lng,lat:this.lat,manage_range_polygon:this.str_lng_lat,grid_member_id:values.grid_member_id,area_id:values.area_id,village_id:values.village_id,single_id:values.single_id,polygon_name:values.polygon_name, fff:2
                            }).then((res) => {
                            ...
                            this.request(streetCommunityApi.saveRange, {
                                id:this.id,grid_member_id:values.grid_member_id,f_area_id:values.area_id,polygon_name:values.polygon_name,f_village_id:values.village_id,bind_id:values.single_id,f_single_id:values.single_id,fid:values.village_id,old_bind_id:this.old_bind_id,type:this.type
                            }).then((res) => {
