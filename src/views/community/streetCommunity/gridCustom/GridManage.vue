      ...
      <bm-control class="bm-control">
        <div class="top">
          <a-row>
              ...
              <div class="top_left">
                <div class="select_button" style="display: flex; align-items: center; justify-content: flex-start; padding-left: 20px;">
                  <div class="color: white;font-size: 14px display: flex; flex-direction: column; justify-content: flex-start;">
                      <div style="color: white;font-size: 14px">选择管理模式：</div>
                      <a-select
                        style="width: 200px;"
                        v-model="select_type"
                        class="select_type"
            ...
            <a-col :span="4">
              <button v-if="select_type == 2" class="start_button" @click="tishi()" style="color: whitesmoke">{{button_name}}</button>
            </a-col>
          </a-row>
        </div>
      </bm-control>
...
<script>
    ...
    import VillageInfoForm from './VillageInfoForm'
    import SingleInfoForm from './SingleInfoForm'
    import VillageRecordInfoForm from './VillageRecordInfoForm'
    import SingleRecordInfoForm from './SingleRecordInfoForm'
        ...
        computed:{
            button_name(){
                if(this.button_type == 1){
                    return '绘制网格范围（当前层级：'+  this.now_zoom + '）';
                                        ...
                                        if(res != ''){
                                            if(res.type == 1){
                                                ...
                                                if(_this.select_type == 2){
                                                    ...
                                                    _this.$refs.singleRecordInfoModal.add(res.single_id,res.street_name+'/'+res.area_name+'/'+res.village_name,res.id);
                                                }
                                            }
                                        }
                                    });
