<template>
    <a-drawer title="编辑" :width="1500" :visible="m_visible" :maskClosable="false" @close="mhandleCancel" placement="right">
        <div class="message-suggestions-list-box">
            <a-card :bordered="false">
                    ...
                    <a-row :gutter="48">
                        <a-col :md="4" :sm="24">
                            <a-input-group compact>
                                <a-input v-model="search.nav_name" placeholder="请输入服务名称" />
                            </a-input-group>
                        </a-col>
                        <a-col :md="2" :sm="24" style="width: 210px;margin-left: 20px;">
                            <a-button type="primary" icon="search" @click="searchList()" style="margin-right: 15px;"> 查
                                询 </a-button>
                        </a-col>

                        <a-col :md="2" :sm="24">
                            <a-button class="add_but" type="primary" icon="plus"
                                @click="$refs.creatAddModal.addBottNavigations(cat_id)">添加数据</a-button>
                        </a-col>
                ...
                <a-table :columns="columns" :data-source="navData" :pagination="pagination" :row-key="record=>record.id"
                    ...
                    <span slot="action" slot-scope="text, record">
                        <a @click="$refs.creatAddModal.editBottNavigation(record.id,cat_id)">编辑</a>
            ...
            <add-bott-service-centre-nav :height='800' :width="1200" ref="creatAddModal" @ok="handleOk" />
        </div>
    </a-drawer>
</template>
...
<script>
    ...
    import addBottServiceCentreNav from './addBottServiceCentreNav.vue';
    const columns = [
            {
                title: 'ID',
                dataIndex: 'id',
    ...
    export default {
        ...
        methods: {
            showList(cat_record){
                this.cat_record=cat_record;
                this.cat_id=cat_record.id;
