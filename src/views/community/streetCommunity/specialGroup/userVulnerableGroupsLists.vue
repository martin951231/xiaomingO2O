    ...
    <a-card :bordered="false">
      <div class="search-box" style="margin-bottom: 10px;">
        <a-row :gutter="48">
          ...
          <a-col :md="8" :sm="24">
            <a-input-group compact>
              <label style="margin-top: 5px;">关怀群体类型：</label>
              <a-select
                      placeholder="请选择关怀群体类型" style="width: 180px;" v-model="search.type_name"
              >
                <a-select-option :value="item.key" v-for="(item,index) in user_label" :key="index">
                  {{item.value}}
                </a-select-option>
              </a-select>
            </a-input-group>
...
<!--          <router-link :to="{ path:'/community/streetCommunity/ThreeLessons/MeetingList' }" style="color: #1890ff;">进入会议列表</router-link>-->
          <router-link :to="{ name:'groupRecord',params:{bind_id:record.id,type:1}}" style="color: #1890ff;">查看记录</router-link>
        </span>
...
<script>
    ...
    export default {
        ...
        computed:{
            columns() {
                ...
                const columns = [
                    ...
                    {
                        title: '所属特殊人员类型',
                        dataIndex: 'user_vulnerable_groups_txt',
                        key: 'user_vulnerable_groups_txt',
                    },
                    {
                        title: '跟踪记录',
                        dataIndex: '',
        ...
        methods: {
          ...
          getUserLabel(){
            this.request(streetCommunityApi.getUserLabel).then((res) => {
              this.user_label=res;
            })
          },
        },
