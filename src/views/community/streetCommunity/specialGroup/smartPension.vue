<template>
  <div class="package-list ant-pro-page-header-wrap-children-content" style="margin:24px 0 0;">
    ...
    <div style="margin-bottom: 5px;">
      <router-link :to="{ path: '/community/streetCommunity/config/index' }">
       <a-alert message="智慧养老数据来源于 仅住户年龄（以身份证号上获取的为准）大于 【街道/社区管理】>【基本信息】中 设置的老年人年龄标准的人员" type="info" />
      </router-link>
    </div>
...
<!--          <router-link :to="{ path:'/community/streetCommunity/ThreeLessons/MeetingList' }" style="color: #1890ff;">进入会议列表</router-link>-->
          <router-link :to="{ name:'groupRecord',params:{bind_id:record.pigcms_id,type:2}}" style="color: #1890ff;">查看记录</router-link>
        </span>
        <a slot="action" slot-scope="text, record" @click="look(record.pigcms_id)">查看</a>
      </a-table>
      <a-drawer
        title="查看"
        :width="720"
    ...
    export default {
        data() {
            return {
                ...
                search: { page: 1,select_option:'name' },
                page: 1,
                detail:{},
            };
        },
