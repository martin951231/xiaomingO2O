const utils = {
	...
	// 生成模拟 3D 饼图的配置项
	getPie3D(pieData, internalDiameterRatio, title='') {
		let series = [];
		let sumValue = 0;
		let startValue = 0;
		let endValue = 0;
		...
		for (let i = 0; i < pieData.length; i++) {
			...
			if (typeof pieData[i].itemStyle != 'undefined') {
				...
				typeof pieData[i].itemStyle.color != 'undefined' ? itemStyle.color = pieData[i].itemStyle
					.color : null;
        ...
        let maxCount = Math.max(...valueArr);
		...
		for (let i = 0; i < series.length; i++) {
			...
			series[i].parametricEquation = utils.getParametricEquation(series[i].pieData.startRatio, series[i]
				.pieData.endRatio,
		...
		let option = {
			...
			grid3D: {
				show: false,
				boxHeight: parseFloat(30/maxCount),
				//top: '30%',
				bottom: '50%',
