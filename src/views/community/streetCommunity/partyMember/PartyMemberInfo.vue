      ...
      <a-tab-pane key="1" tab="基本信息" class="tab-panes">
        <a-spin :spinning="confirmLoading"  :height="800" >
            ...
            <a-form-item label="身份证号" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-col :span="18">
                <a-input v-decorator="['id_card', {initialValue:detail.id_card,rules: [{required: true, message: '请输入身份证号！'}]}]" />
              </a-col>
              <a-col :span="6">
              </a-col>
            </a-form-item>
            ...
            <a-form-item label="党员状态" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-col :span="50">
                  <a-radio-group  v-decorator="['party_status',{initialValue:detail.party_status}]">
                    <a-radio :value="1">正常</a-radio>
                    <!--<a-radio :value="2">转出</a-radio>-->
                    <!--<a-radio :value="3">失联</a-radio>-->
                    <a-radio :value="4">死亡</a-radio>
                    <a-radio :value="5">退党</a-radio>
                  </a-radio-group>
              </a-col>
            </a-form-item>
            <a-form-item label="入党时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-col :span="18">
                <a-date-picker placeholder="入党时间" :value="date_moment(detail.join_party_time, dateFormat)" @change="onChange"></a-date-picker>
              </a-col>
            </a-form-item>
              ...
              <a-col :span="18">
                <a-row>
                  <div>
                    ...
                    <div style="float: right">建议尺寸：295*365px</div>
                  </div>
                </a-row>
              </a-col>
            </a-form-item>
            <a-form-item label="是否是优秀党员" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-col :span="18">
                <a-radio-group v-decorator="['is_good_party',{initialValue:detail.is_good_party}]" >
      ...
      <a-tab-pane key="2" tab="业主资料" class="tab-panes">
        <a-spin :spinning="confirmLoading"  :height="800">
            <a-form-item v-for="(item,key) in dataList" :label="item.title" :labelCol="labelCol" :wrapperCol="wrapperCol" :key="key">
              <a-col :span="18">
                <!-- 文本 -->
                <a-input v-if="item.type == 1" name="data[item['key']]" v-decorator="[item['key'],{initialValue:detail.authentication_field[item['key']]['value']}]"/>

                <!--单选-->
                <a-select v-if="item.type == 2" v-decorator="[item['key'],{initialValue:detail.authentication_field[item['key']].value}]">
                  <a-select-option v-for="(val,i) in item.use_field" :value=val :key="i">{{val}}</a-select-option>
                </a-select>
                <!--籍贯-->
                <a-select v-if="item.type == 3" v-model="detail.authentication_field[item['key']].province" style="width: 120px" @change="handleProvinceChange">
                  <a-select-option v-for="item in provinceData" :value="item.id" :key="item.name">{{item.name}}</a-select-option>
                </a-select>
                <a-select v-if="item.type == 3 && cityData" v-model="detail.authentication_field[item['key']].city" style="width: 120px" @change="handleCityChange">
                  <a-select-option v-for="city in cityData" :value="city.id" :key="city.name">
                    {{ city.name }}
                  </a-select-option>
                </a-select>
                <!--日期-->
                <a-date-picker :format="dateFormat" placeholder="出生日期"  v-if="item.type == 4" :value="date_moment(detail.authentication_field[item['key']].value, dateFormat)" @change="onChangeBorn"></a-date-picker>
              </a-col>
            </a-form-item>
        </a-spin>
      </a-tab-pane>
        ...
        <a-spin :spinning="confirmLoading"  :height="800">
          <!--   政治面貌       -->
            <a-form-item :label="political_affiliation_title" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-col :span="50">
                <a-radio-group :value="political_affiliation_val" id="radio-labels" @change="onClickPolitical">
                  <a-radio v-for="(item0,key0) in political_affiliation_arr" :value="key0" :key="key0">{{item0}}</a-radio>
                </a-radio-group>
                <a-select v-if="is_show_party_type" style="width: 140px" v-model="party_type" @change="onClickPartyType">
                  <a-select-option :value="0" >请选择党员属性</a-select-option>
                  <a-select-option :value="1" >社区党员</a-select-option>
                  <a-select-option :value="2" >居住地党员</a-select-option>
                </a-select>
                <a-select v-if="is_show" style="width: 200px" v-model="detail.party_id">
                  <a-select-option :value="0" >请选择党支部</a-select-option>
                  <a-select-option v-for="(val0,i0) in party_branch_list" :value="val0.id" :key="i0">{{val0.name}}</a-select-option>
                </a-select>
                <div v-if="is_show_party_type_msg" style="float: right">社区党员是指：组织关系在本社区的党员；居住地党员是指：组织关系不在社区，但居住在本社区的党员，其中包括流动党员</div>
              </a-col>
              <a-col :span="6">
              </a-col>
            </a-form-item>
          <!--   特殊人群       -->
            <a-form-item :label="special_groups_title" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-col :span="50">
                  <a-checkbox-group name="user_special_groups"  :value="detail.user_special_groups" @change="onChangeSpecial">
          ...
          <!--   重点人群       -->
            <a-form-item :label="focus_groups_title" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-col :span="18">
                <a-checkbox-group name="user_focus_groups" :value="detail.user_focus_groups" @change="onChangeFocus">
            ...
            <a-form-item :label="vulnerable_groups_title" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-col :span="18">
                <a-checkbox-group name="user_vulnerable_groups" :value="detail.user_vulnerable_groups" @change="onChangeVulnerable">
    ...
    export default {
        data() {
            ...
            return {
                ...
                native_place_p:'',
                native_place_c:'',
                is_show_party_type:false,
                is_show_party_type_msg:false,
                party_type:0,
                party_bind_user_id:0,
            }
        },
        mounted() {
        },
        methods: {
            ...
            onClickPolitical(e){
                ...
                if(e.target.value == 1){
                    this.is_show_party_type = true;
                    if(this.party_type == 1){
                        this.is_show = true;
                    }
                    this.is_show_party_type_msg = true;
                }else{
                    this.is_show_party_type = false;
                    this.is_show = false;
                    this.is_show_party_type_msg = false;
                }
            },
            onClickPartyType(e){
                if(this.party_type == 1){
            ...
            handleProvinceChange(value,key) {
                ...
                this.request(streetCommunityApi.getCity,{id:value,type:1})
                    .then((res) => {
            ...
            edit(id,party_bind_user_id) {
                this.visible = true
                this.id = id
                this.party_bind_user_id = party_bind_user_id
                this.getEditInfo()
                console.log(this.id)
                this.title = '编辑'
                this.provinceData = [
            ...
            handleSubmit() {
                ...
                validateFields((errors, values) => {
                    if (!errors) {
                        ...
                        if(!this.native_place_p && !this.native_place_c && this.detail.authentication_field.native_place)
                        {
                        ...
                        if(!this.born_time && this.detail.authentication_field.birthday){
                            values.birthday = this.detail.authentication_field.birthday.value
                        }else{
                            values.birthday = this.born_time;
                        }
                        values.party_type = this.party_type;
                        values.party_bind_user_id = this.party_bind_user_id;
                        console.log(values);
            ...
            getEditInfo() {
                ...
                this.request(streetCommunityApi.getPartyMemberInfo, {
                    id: this.id
                })
                    .then((res) => {
                        ...
                        if (typeof(res.info)=='object') {
                            ...
                            this.party_type = res.info.party_type;
                        }
                        if(res.info.authentication_field && res.info.authentication_field.native_place && res.info.authentication_field.native_place.province){
                            this.request(streetCommunityApi.getCity,{id:res.info.authentication_field.native_place.province})
                                .then((res) => {
                                    if(res.error==0){
                                        if(res.list){
                                            this.cityData = res.list;
