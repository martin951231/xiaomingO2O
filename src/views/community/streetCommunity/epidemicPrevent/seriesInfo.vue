            ...
            <a-form :form="form" class="prepaid_info">
                ...
                <a-form-item label="" :labelCol="labelCol" :wrapperCol="wrapperCol">
                    ...
                    <a-input
                            :maxLength="5"
                            style="width: 300px"
                            placeholder="请输入名称（最多五个字）"
                            v-decorator="['title',{ initialValue: post.title,rules: [{ required: true, message: L('请输入名称（最多五个字）！') }] }]"
                    />
                </a-form-item>
        ...
        methods: {
            ...
            handleSubmit() {
                ...
                validateFields((errors, values) => {
                    if (!errors) {
                        let url=streetCommunityApi.epidemicPreventSeriesAdd;
                        if(this.post.id > 0){
                            url=streetCommunityApi.epidemicPreventSeriesSub;
                        }
            ...
            getEditInfo() {
                this.confirmLoading = true;
                this.request(streetCommunityApi.epidemicPreventSeriesOne, {id: this.post.id}).then((res) => {
                    this.post=res;
                    this.confirmLoading = false;
                    this.visible = true;
                })
