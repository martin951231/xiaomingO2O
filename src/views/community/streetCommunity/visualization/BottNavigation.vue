<template>
  <a-modal :title="title" :width="1200" :footer="null" :visible="visible" :maskClosable="false" :confirmLoading="confirmLoading" @cancel="handleCancel">
    ...
    <span class="add-banner">
          <a @click="$refs.createModal.addBottNavigations()">添加导航</a>
        </span>
    <hr/>
    <div class="prompt">图标建议尺寸：图片宽度建议为：150px，高度建议为：150px</div>
    <a-card :bordered="false" >
      <a-table :columns="columns" :data-source="navigationList"
               :pagination="pagination"
               @change="tableChange" >
        <span slot="action" slot-scope="text, record">
          <a @click="$refs.createModal.editBottNavigation(record.id)">编辑</a>
    ...
    <add-bott-navigation :height='800' :width="1200" ref="createModal" @ok="handleOk"/>
  </a-modal>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            streetLists() {
                ...
                let _this = this
                this.request(streetCommunityApi.getStreetNavList, this.search)
                    .then((res) => {
                        console.log('res', res)
                        this.navigationList = res.list;
            ...
            deleteConfirm(id){
                this.request(streetCommunityApi.streetNavDel,{'id':id})
                    .then((res) => {
                        this.streetLists()
