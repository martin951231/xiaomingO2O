        ...
        <div class="buy-box" v-if="type==1">
            <div class="buy-parent">
                <div class="buy-divs" id="buy-pics">
                    ...
                    <div class="buy-room-package" v-for="(item,index) in roomList" :key="index" 
                    >
                        <div class="buy-room-box" :class="{ 'active': item.pitch_type == 1 }" :id="'box-'+index" @click="choose_room_data($event,index,item.room_id)">
...
<script>
    ...
    { title: '套餐所含'+store.getters.config.room_name, dataIndex: 'room_num', key: 'room_num' },
    { title: '套餐价格（元/年）', dataIndex: 'package_price', key: 'package_price' },
    { title: '续费周期', dataIndex: 'package_num', key: 'package_num'},
    ...
    { title: '套餐所含'+store.getters.config.room_name, dataIndex: 'room_num', key: 'room_num' },
    { title: '套餐价格（元/年）', dataIndex: 'package_price', key: 'package_price' },
    { title: '额外购买'+store.getters.config.room_name+'数', dataIndex: 'pay_room_total_num', key: 'pay_room_total_num' },
    { title: '额外购买'+store.getters.config.room_name+'金额', dataIndex: 'room_total_price', key: 'room_total_price' },
    { title: '总价', dataIndex: 'total_money', key: 'total_money'},
    ];
    export default {
                ...
                this.request(packageApi.propertyGetPackageRoom, param)
                    .then((res) => {
                        ...
                        if (res.package) {
                            ...
                            this.pay_room_total_num = res.package.pay_room_total_num;
                        }
                        if (res.room_list) { 
                            ...
                            for(var i in res.room_list) {
                                ...
                                if(res.room_list[i].pitch_type == 1) {
                                    if (res.package.pay_room_total_num > 0 && res.room_list[i].room_count < res.package.pay_room_total_num) {
                                        let room_num = Math.ceil(res.package.pay_room_total_num / res.room_list[i].room_count);
                                        // let room_num = Math.ceil(res.room_list[i].room_count/res.package.pay_room_total_num);
                                        res.room_list[i]['room_num'] = room_num;
                                    }
                                }
                            }
                        }
            ...
            handleSubmit() {
                ...
                for(var i in roomList) {
                    ...
                    if (roomList[i] && roomList[i]['room_num'] > 0  && choose_room[i] > 0) {
                        ...
                        if(choose_room.length<=0 && roomList[i].pitch_type == 1)
                        {
                            console.log('dddddddddddd',roomList[i]);
                            room_single = {};
                            room_single['room_id'] = roomList[i]['room_id'];
                            room_single['room_num'] = roomList[i]['room_num'];
                            room.push(room_single);
                ...
                console.log('package_info_room_num',this.package_info.room_num);
                console.log(this.pay_room_total_num);
                if (len>0 || this.package_info.room_num>=this.pay_room_total_num) {
                    ...
                    this.$notification.warning({
                        message: '升级套餐的'+store.getters.config.room_name+'数不能小于现有套餐'+store.getters.config.room_name+'数！',
                    })
                    this.confirmLoading = false;
