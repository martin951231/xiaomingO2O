<template>
  ...
  <div class="box">
    <div class="parent">
   <!-- <div class="left_arr">
        <img class="left_arr_img" src="https://hf.pigcms.com/static/wxapp/images/left_arr_icon.png" >
    </div>
    <div class="right_arr">
        <img class="right_arr_img" src="https://hf.pigcms.com/static/wxapp/images/right_arr_icon.png" >
    </div>  -->
    <!-- @mousedown="handlemouse" @mouseleve="leave"
           @click="clickDoSome(index)" -->
    <div class="divs" id="pics">
      <div class="package" v-for="(item,index) in packageData" :key="index" 
      >
        <div class="package-top" style="text-align: -webkit-center;">
    ...
    export default {
        ...
        methods: {
            packageList() {
                ...
                this.loadPost = true;
                this.request(packageApi.propertyGetOrderPackage)
                    .then((res) => {
                        this.loadPost = false;
                        console.log('res', res)
            ...
            packageRenewal(package_data,buy_periods, type) {
                let param = {};
                let _this = this;
                param['package_id'] = package_data.package_id;
                param['package_num'] = buy_periods;
                param['type'] = type;
                param['order_id'] = package_data.order_id;
                this.request(packageApi.propertyGetPackageRoom, param)
                    .then((res) => {
                        console.log('info',res);
                        if(res.status != '-1'){
                            //调转 start
                            var pathInfo = this.getRouterPath('property_packages_room_order_buy')
