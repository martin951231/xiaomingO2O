    ...
    <a-drawer   :width="1000" :visible="visible"  @close="handleSubCancel">
        <div>
            ...
            <a-tabs default-active-key="1">
                ...
                <a-tab-pane key="1">
                    <span slot="tab">  <a-icon type="alert" />  详细日志  </span>
                    <div>
...
export default{
    ...
    methods: {
        ...
        getDetailInfo(){
            let that = this
            if(that.log_fid){
                that.request(propertyApi.loginLogDetail, {log_fid: that.log_fid}).then((res) => {
                    that.logExtend = res.logInfo
                })
            }
        },
        handleSubCancel(e) {
