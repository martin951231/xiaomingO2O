    ...
    <a-spin :spinning="confirmLoading"  :height="800">
      <a-form :form="form">
        <a-form-item label="分组名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-col :span="18">
            <a-input :maxLength="15" placeholder="请输入分组名称（分组名称不得超过15个字）" @change="text_change" v-decorator="['name', {initialValue:detail.name,rules: [{required: true, message: '请输入分组名称！'}]}]" />
          </a-col>
          <a-col :span="6">
          </a-col>
        </a-form-item>
            ...
            handleSubmit() {
                ...
                validateFields((errors, values) => {
                    if (!errors) {
                        ...
                        this.request(configVillageApi.subCodeGroup, values)
                            .then((res) => {
                                if (this.detail.id>0) {
