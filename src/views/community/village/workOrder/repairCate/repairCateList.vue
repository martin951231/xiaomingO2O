<template>
  <a-modal width="1000px" title="分类管理" v-model="bindVisible" :footer="null" :maskClosable="false" @cancel="handleCandel">
    <div class="package-list" >
      <a-card :bordered="false">
        <div class="table-operator" style="margin-bottom: 10px">
          <a-button type="primary" icon="plus" @click="$refs.createModalsss.add(0,subject_id)">添加分类</a-button>
        </div>
        <a-table :columns="columns" :data-source="list"
          ...
          <a @click="$refs.createModal.childList(record)">查看子分类</a>
        </span>
...
<script>
  ...
  import cateList from './repairCateChildList.vue'
  ...
  export default {
    ...
    methods: {
      ...
      getCateList() {
        this.request(configVillageApi.getCateList, {'page': this.page,'subject_id':this.subject_id,'parent_id':0})
          .then((res) => {
            console.log('res', res);
            this.list = res.list;
