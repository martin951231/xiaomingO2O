<template>
    <div class="manual_open">
        ...
        <div class="header_search" style="padding-top: 0;">
            <a-button type="primary" :loading="exportLoadding" @click="exportThis" v-if="role_export4car==1">excel导出</a-button>
        </div>
        
        <div class="table_content">
            <a-table :columns="park_sys_type=='A11'?columns1:columns" :data-source="manualList" :row-key="record=>record.record_id" :pagination="pageInfo" :loading="tableLoadding"
            @change="handleTableChange">
                <!-- <span slot="action" slot-scope="text, record">
                    <a @click="editThis(record)">详情</a>
                </span> -->
            </a-table>
            <manual-edit :manual_id="manual_id" :visible="manualVisible" :modelTitle="modelTitle" @closeManual="closeManual"/>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            getManualList(){
                ...
                that.request(villageApi.getOpenGateList, that.pageInfo).then((res) => {
                    that.manualList = res.list
                    that.pageInfo.total = res.count
                    that.park_sys_type=res.park_sys_type
                    that.tableLoadding = false
                    if(res.role_export4car!= undefined){
                        this.role_export4car=res.role_export4car;
                    }else{
                        this.role_export4car=1;
                    }
                }).catch(e=>{
                    that.tableLoadding = false
                })
            ...
            editThis(record){
                this.manual_id = record.record_id+''
            ...
            exportThis(){
                ...
                that.request('/community/village_api.Parking/downOpenGate', that.pageInfo).then((res) => {
                    if(res.error == 0){
