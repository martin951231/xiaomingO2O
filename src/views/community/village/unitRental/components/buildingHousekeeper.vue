<template>
    <a-drawer :title="xtitle" :visible="visible" :width="850" @close="resetForm" :body-style="{ paddingBottom: '80px' }">
        <a-form-model ref="ruleForm" :model="buildForm" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
            <a-form-model-item label="绑定员工" prop="yuangong" extra="多选，不限制数量，绑定员工后，业主加好友时随机从其中选择">
                <a-select mode="multiple" placeholder="请选择员工"
                    @change="(value)=>handleSelectChange(value, 'yuangong')" :value="selectedItems">
                    <a-select-option v-for="(item, index) in filteredOptions" :value="item.name" :index="index" :key="index">
                        {{item.name}}</a-select-option>
                    </a-select>
            </a-form-model-item>
            <a-form-model-item v-if="floor_id<=0" :label="'绑定'+$store.getters.config.single_name" prop="is_kefu" :extra="'选择“是”选项，添加客服时只能选择'+$store.getters.config.single_name+'下的业主；选择“否”选项，添加客服时能选择'+$store.getters.config.single_name+'所有的业主'">
            ...
            <a-form-model-item label="装修模板" prop="zhuangxiu">
                <template slot="extra">
                ...
                <a-select placeholder="请选择装修模板" style="width: 300px" :value="buildForm.template_type"
                    @change="(value)=>handleSelectChange(value, 'zhuangxiu')">
                    <a-select-option v-for="(item, index) in repaireList" :value="item.id" :index="index" :key="index">{{item.name}}</a-select-option>
                </a-select>
            </a-form-model-item>
            <a-form-model-item v-if="buildForm.template_type == 2" label="上传模板1" prop="template_url" extra="尺寸：750*1334；注意：模板左下角需距离页面边缘间距20像素预留100*100像素的空白位置，用于放置群二维码">
                <a-upload name="reply_pic" :multiple="false" :action="uploadUrl" :data="uploadParams" :before-upload="beforeUpload" :showUploadList="false"
            ...
            <a-form-model-item v-if="effect_img" label="效果预览">
                <img style="width: 50px;" :src="effect_img">
            </a-form-model-item>
        </a-form-model>
        ...
        <a-modal title="查看图片" :width="350" :visible="previewVisible" @cancel="handlePreviewCancel" :footer="null">
            <div style="width: 100%; display: flex; justify-content: center; align-items: center;">
...
<script>
    ...
    export default defineComponent({
        ...
        setup(props, context) {
            ...
            let xtitle = ref(store.getters.config.single_name+'管家')
            //表单保存
            const onSubmit = () => {
            ...
            const resetForm = () => {
                ...
                buildForm.value = {template_type:0}
                imageUrl.value = ''
                ercodeUrl.value = ''
                effect_img.value = ''
            ...
            const saveForm = () => {
                ...
                Vue.prototype.request(villageNew.saveUnitRentalButler, buildForm
                    .value).then((res) => {
            ...
            const getBuildInfo = (single_id) => {
                Vue.prototype.request(villageNew.getUnitRentalButler, {
                    single_id,floor_id:props.floor_id
                }).then((res) => {
            ...
            const handleSelectChange = (value, type) => {
                ...
                if (type == 'zhuangxiu') {
                    ...
                    if (value == 1 && buildForm.value.qy_qrcode) {
                        services_look_img();
                    } else if (value2 == 2 && buildForm.value.qy_qrcode && buildForm.value.template_url){
                        services_look_img();
                    } else {
            ...
            const services_look_img = () => {
                ...
                Vue.prototype.request(villageNew.servicesImgPreview, {
                    ...
                    console.log('services_look_img',data);
                    if (data.data) {
            ...
            watch(() => props.visible, (val) => {
                if (val) {
                    props.floor_id=parseInt(props.floor_id) ;
                    if(props.floor_id>0){
                        xtitle.value=store.getters.config.floor_name+'管家'
                    }
