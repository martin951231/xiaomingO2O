<template>
    <a-drawer :title="title" :visible="visible" :width="1200" @close="resetForm(false)" :body-style="{ paddingBottom: '80px' }">
        <a-form-model ref="ruleForm" :model="buildForm" :rules="rules" :label-col="labelCol"
           :wrapper-col="wrapperCol">
           <div style="display: flex;">
               <a-card title="基本信息" style="width: 600px">
                   ...
                   <a-form-model-item>
                        <template slot="label">
                            ...
                            {{$store.getters.config.single_name}}地址
                        </template>
               ...
               <a-card :title="$store.getters.config.single_name+'资料'" style="width: 600px">
...
<script>
    ...
    export default defineComponent({
        ...
        setup(props, context) {
            ...
            const buildForm = ref({})
            const title = ref(store.getters.config.single_name+'编辑')
            
            const rules = ref({
                single_name: [{ required: true, message: '请输入名称', trigger: 'blur' }],
                single_number: [{ required: true, message: '请输入编号', trigger: 'blur' }],
                long_lat: [{ required: true, message: '请选择地址', trigger: 'blur' }],
                contract_time: [{ required: true, message: '请选择合同时间', trigger: 'blur' }],
                sort: [{ required: true, message: '请输入排序', trigger: 'blur' }],
                measure_area: [{ required: true, message: '请输入面积', trigger: 'blur' }],
            ...
            const onSubmit = () => {
                ruleForm.value.validate(valid => {
                    if (valid) {
                        if(!buildForm.value.long_lat){
                            Vue.prototype.$message.warn('请选择经纬度')
                            return false;
                        }
                        buildForm.value.single_id = buildForm.value.id
                        if(!buildForm.value.contract_time_start || !buildForm.value.contract_time_end){
                            Vue.prototype.$message.warn('请选择合同时间')
                            return
                        }
            ...
            const saveForm = () => {
                Vue.prototype.request(villageNew.updateUnitRentalInfoByID, buildForm
                    .value).then((res) => {
            ...
            const getBuildInfo = (single_id) => {
                Vue.prototype.request(villageNew.unitRentalInfo, {
                    single_id,
                }).then((res) => {
                    buildForm.value = res
            ...
            watch(() => props.visible, (val) => {
                if(val){
                    if(props.single_id>0){
                        title.value='编辑' + store.getters.config.single_name;
                    }else{
                        title.value='添加' + store.getters.config.single_name;
                    }
