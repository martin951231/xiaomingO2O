<template>
    <div class="fee-summary-list-box">
        <div class="search-box">
            <a-row :gutter="48">
                <a-col :md="8" :sm="24" style="padding-left: 5px;padding-right: 1px;width: 500px">
    ...
    export default {
        ...
        data() {
            return {
                ...
                project_rule_list: [],
                project_list: [],
                search_data: [ moment().format('YYYY-MM-DD') , moment().format('YYYY-MM-DD') ],
                xscroll:{x: 1400},
                tips_visible:false,
        ...
        methods: {
            ...
            getOrderTableProjectList() {
                this.request(villageApi.getOrderTableProjectList, {
                        'pay_status': 'is_pay',
                        'xtype':'daily',
                    })
                    .then((res) => {
                        if (res.lists && res.lists.length > 0) {
                            ...
                            for(let ii=0;ii<res.lists.length;ii++){
                                this.columns.push({
                                    title: res.lists[ii].name,
                                    dataIndex: 'project_'+res.lists[ii].id,
                                    key: 'project_'+res.lists[ii].id,
                                    width:140
                                });
                            }
