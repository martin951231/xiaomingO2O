  ...
  <div class="container_box">
    ...
    <div class="container_child_box">
      ...
      <div class="right_box" v-if="is_selected">
        ...
        <a-table v-if="freashTable" :pagination="false" :row-selection="rowSelection" rowKey="order_id" :columns="columns" :data-source="data" :scroll="{ x: 1100 }">
          <a slot="name" slot-scope="text">{{ text }}</a>
          <span slot="action" slot-scope="text, record">
            <a @click="showpopup_detailsList(record)">详情</a>
            <a-divider v-if="record.show_action == 1" type="vertical" />
            <a v-if="role_mdymoney==1 &&record.show_action == 1 && (!record.my_check_status || record.my_check_status==0)" @click="showpopupe_revise(record,record.order_id)">修改费用</a>
            ...
            <a-divider type="vertical" v-if="(!record.my_check_status || record.my_check_status==0) && role_discard==1" />
            
            <span v-if="(!record.my_check_status || record.my_check_status==0) && role_discard==1">
              <a class="ant-dropdown-link" @click="invalidShow(0,record,record.order_id)" v-if="record.show_action == 1">作废账单</a>
    ...
    <a-modal
      ...
      <a-table :pagination="false" :columns="columns_2" :data-source="data_detail">
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a v-if="role_discard==1 && (!record.my_check_status || record.my_check_status==0)" class="ant-dropdown-link" @click="invalidShow(type=2,record,record.order_id)">作废账单</a>
  ...
  export default {
    ...
    data () {
      return {
        ...
        cycle_set: '', // 周期单位
        cycle: '',// 缴费周期
		config_customization:{life_tools:-1},
        showScroll: false,
        role_addcar:0,
        role_addorder:0,
        role_addrule:0,
          ...
          onChange: (selectedRowKeys, selectedRows) => {
            ...
            if (selectedRows) {
              ...
              for (var i = 0; i < selectedRows.length; i++) {
                  if(selectedRows[i].detail_order.length>1){
                      for (var ii = 0; ii < selectedRows[i].detail_order.length; ii++) {
                          oids.push({
                            orderid: selectedRows[i].detail_order[ii]['order_id'],
                            pigcms_id: selectedRows[i].detail_order[ii]['pigcms_id'],
    ...
    methods: {
        expressBill(type){
          ...
          if (type!=1 && _this.default_key.length<1) {
            this.$message.warning('请选择'+store.getters.config.room_name+'或车位')
            return false
          }
          if(type!=1 &&  _this.default_key.length>0  && _this.default_key[0].split('|')){
      ...
      getCheckauthDetail () {
          this.loading = true
          this.request(villageApi.getCheckauthDetail, { order_id: _this.record_data.order_id, check_apply_id: _this.record_data.check_apply_id, xtype: 'order_discard', page: 1 })
              .then((res) => {
      ...
      getOrderList (preKey) {
        this.request(villageApi.getOrderList, {
          ...
          if (res) {
            ...
            _this.show_scrcu=res.show_scrcu!==undefined ? res.show_scrcu:1
          }
        })
      },
      // 收费项列表
