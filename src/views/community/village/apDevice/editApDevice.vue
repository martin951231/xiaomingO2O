        ...
        <a-spin :spinning="confirmLoading" >
            <a-form :form="form">
                    ...
                    <a-form-item label="设备编号" :labelCol="labelCol" :wrapperCol="wrapperCol" >
                        <a-col :span="24" >
                            <a-input placeholder="请输入设备编号" v-model="post.ac_num" :disabled="true"/>
                        </a-col>
                    </a-form-item>
                ...
                <a-form-item label="设备位置" :labelCol="labelCol" :wrapperCol="wrapperCol" :required="true">
                    <a-col :span="10" >
                        <a-select default-value="1" style="width: 120px" :placeholder="'请选择'+$store.getters.config.single_name+'或公共区域'" v-model="post.area_type" @change="getAreaList1()">
                ...
                <a-form-item label="启用时间" :labelCol="labelCol" :wrapperCol="wrapperCol"  :required="true">
                    <a-col :span="24" >
                        <a-date-picker  style="width: 300px" format="YYYY-MM-DD"  :allowClear="false" @change="dayOnChange" placeholder="请选择日期" :value="moment(post.use_time, 'YYYY-MM-DD')" />
                    </a-col>
                </a-form-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getInfo(id){
                this.request(villageApi.getDeviceInfo,{id:id}).then((data) => {
                    if (data){
            ...
            getAreaList(id){
                this.request(villageApi.getAddressList,{id:id}).then((res) => {
                    if (res){
            ...
            handleSubmit() {
                if(this.post.id > 0){
                    let that = this
                    this.request(villageApi.editDevice,this.post).then((data) => {
                        if (data){
