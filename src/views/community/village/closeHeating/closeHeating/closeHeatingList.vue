<template>
    <div class="parking_space">
        <div class="header_search">
            ...
            <div class="search_item" style="margin-left: 20px;">
                <label class="label_title">是否符合供暖：</label>
                <a-select show-search placeholder="请选择" style="width: 200px" :filter-option="filterOption"
                          @change="handleSelectChange" v-model="pageInfo.is_measure_up">
            ...
            <div class="search_item">
                <a-button type="primary" v-if="role_add" @click="$refs.detailModal.add()">申请停暖</a-button>
                <a-button type="primary" v-if="role_protocol" @click="$refs.ProtocolModel.set()" class="operation_btn">停暖协议</a-button>
                <a-button type="primary" v-if="role_set" @click="$refs.NoticeModel.set()" class="operation_btn">账单提醒设置</a-button>
            </div>
        ...
        <notice-set ref="NoticeModel" />
        <close-heating-detail ref="detailModal" @ok="getDataList" />
    </div>
</template>

<script>
    import villageApi from '@/api/community/village';
    import ProtocolSet from '@/views/community/village/closeHeating/closeHeating/protocolSet'
    import NoticeSet from '@/views/community/village/closeHeating/closeHeating/noticeSet'
    import CloseHeatingDetail from '@/views/community/village/closeHeating/closeHeating/closeHeatingDetail'
    const columns = [{
            title: '编号',
            dataIndex: 'sn',
            key: 'sn',
        ...
        {
            title: '是否符合供暖条件',
            dataIndex: 'is_measure_up_msg',
            key: 'is_measure_up_msg',
            scopedSlots: {
                customRender: 'str'
            },
        },
    ...
    export default {
        ...
        methods: {
            ...
            delConfirm(record) {
                let that = this
                that.request(villageApi.delCloseHeating, {
                ...
                that.$confirm({
                    ...
                    onOk() {
                        that.request(villageApi.delAllParkPosition, {
                            pigcms_id: that.selectedRowKeys
                        }).then((res) => {
                            that.$message.success('删除成功！')
                            that.selectedRowKeys = []
                            that.getDataList()
