<template>
    <div class="current_2">
        <div class="equipPoint_container">
            <div class="draw_canvas" v-if="IntelligentEqu.is_photo*1==1 && villagePhoto" ref="canvas_con" @click="selectPoint">
                <img v-if="IntelligentEqu.is_photo*1==1" class="back_image" :src="villagePhoto"
                    :style="{width:canvasProp.width, height: canvasProp.height}" />
                <canvas v-if="IntelligentEqu.is_photo*1==1" class="my_canvas" id="canvasIntelligent" :width="canvasProp.width" :height="canvasProp.height"></canvas>
            </div>
            <div v-if="IntelligentEqu.is_photo*1!=1 || !villagePhoto" class="draw_canvas">
                <span v-if="$store.getters.config.house_name">你还没有上传{{$store.getters.config.house_name}}平面图</span>
        ...
        <div class="left_box_tab">
            <div class="left_box_tab_head"></div>
            <div class="left_box_tab_list" v-for="(item, index) in btnList" :key="index" @click="chooseBtn(item, index)"
        ...
        <div class="right_content">
            <!-- <div class="right_content_top">
                <div class="right_content_top_btn" :class="currentIndex2 == index? 'active': ''"
                    v-for="(item, index) in btnList" :key="index" @click="chooseBtn(item, index)">{{item.name}}</div>
            </div> -->
            ...
            <a-modal title="选择子设备类型"
                ...
                :footer="null">
                <span>选择子设备：</span>
                <a-select mode="multiple" style="width: 200px" :token-separators="[',']" @change="handleChange" placeholder="请选择子设备" v-if="childEquipVisible">
        ...
        <!-- 弹窗 -->
        <PopupBox ref="PopupBox" title="设备点位信息">
            <div class="popup_content">
                ...
                <div class="flex_box_2">
                    <div class="flex_column flex_mini_box_1">
                        <!-- <img class="popup_icon_1" src="../../village/images/cockpit_shequ.png" alt=""> -->
                        <div class="popup_icon_1" :style="{backgroundColor: detailType.color}"></div>
                        <p class="popup_text_2">{{detailType.title}}</p>
                    </div>
                </div>
                <div class="flex_row flex_box_1">
                    <div class="square"></div>
                    <p class="popup_text_1">基本信息</p>
                </div>
                <div class="flex_box_3">
                    <div class="text" v-for="(item, index) in detailList" :key="index">{{item.key}}:
        ...
        <!-- 一个点位对应多个设备时的处理方法 -->
       <!-- <a-modal
          title="该点位绑定了多个设备,请选择对应设备查看"
          :visible="showEquipList"
          @cancel="handleCancel">
            <a-select show-search placeholder="请选择一个设备"
                v-if="showEquipList"
                option-filter-prop="children" style="width: 200px" :filter-option="filterOption"
                @change="handleChange">
                <a-select-option default-value="" :value="item.id" v-for="(item,index) in equipList" :key="index">
                    {{item.device_name}}
                </a-select-option>
            </a-select>
    ...
    export default {
        ...
        data() {
            return {
                ...
                btnList: [{
                    name: store.getters.config.house_name+'平面图'
                }, {
                    name: '地下一层'
                }, {
                    name: '地下二层'
                ...
                detailType: {},
                detailList: [],
                imageUrl: 'https://hf.pigcms.com/static/wxapp/images/map_picture.png',
        ...
        mounted() {
            let that = this
            that.requestParams.village_id = 50
            let timeout = setTimeout(() => {
                ...
                console.log("that.canvasProp========>", that.canvasProp)
                that.requestParams.type = 4
                that.getFacilitiesData()
        ...
        methods: {
            ...
            selectPoint($el) {
                ...
                this.coordinateList.map(v => {
                    if (v.coordinate && flag) {
                        ...
                        if (distanceX - 10 < pointX && pointX < distanceX + 10 && distanceY - 10 < pointY &&
                            pointY < distanceY + 10) {
                            this.equipList.push(v)
                            console.log("v=========>", v)
                            this.getIntelligentInfo(v.id, v.device_id)
                ...
                //     this.getIntelligentInfo(this.equipList[0].id, this.equipList[0].device_id)
                //     this.$refs.PopupBox.open()
                // } else {
            ...
            // handleOk(){
            //     if(this.equipDetail.id) {
            //         this.getIntelligentInfo(this.equipDetail.id, this.equipDetail.device_id)
            //         this.$refs.PopupBox.open()
            //         this.showEquipList = false;
            //         this.equipDetail = {}
            //     } else {
            //         this.$message.info('请先选择一个设备！')
            //     }
            // },
            
            // handleChange(value){
            //     this.equipList.map(v=>{
            //         if(v.id == value){
            //             this.equipDetail = {
            //                 id: v.id,
            ...
            initDevicePoint() {
                let canvas = document.getElementById('canvasIntelligent');
                let ctx = canvas.getContext('2d');
                // ctx.fillStyle="#409EFF";
                this.coordinateList.map((v, i) => {
                    if (v.coordinate) {
                        let img = new Image()
                        if (v.img) {
                            img.src = v.img
            ...
            clearCanvas() {
                if(document.getElementById("canvasIntelligent")){
                    let c = document.getElementById("canvasIntelligent");
                    let cxt = c.getContext("2d");
                    cxt.clearRect(0, 0, 1000, 933);
                }
            },
            ...
            chooseInt(item, index) {
                ...
                if (this.currentIndex3 == index) {
                    ...
                    this.statusName = ''
                    // if(item.sub_series_type){
                    //     this.requestParams.sub_series_type = item.sub_series_type
                    // }
                    // if(item.sub_series_type && item.sub_series_type*1==5){
                    //     this.requestParams.series_key = item.series_key
                    // }
                    this.getFacilitiesData()
                }
            },
            ...
            getFacilitiesData() {
                this.request(configVillageApi.getFacilitiesData, this.requestParams, 'post').then((res) => {
                    this.coordinateList = []
                    this.IntelligentEqu = res
            ...
            getIntelligentInfo(id, device_id) {
                this.request(configVillageApi.getFacilitiesDetails, {
                    id: id,
                    device_id: device_id
                }, 'post').then((res) => {
                    this.detailList = res.list
                    this.detailType = res.type
                })
            },

            navigateTo() {
                this.$router.push({
                    path: this.IntelligentEqu.jump_url
                })
            },
            handleChildOk(){
            ...
            .right_content_bottom {
                ...
                .content_bottom_middle {
                    ...
                    .bottom_middle_con {
                        ...
                        width: 96%;
                        height: 660/@rem * 1rem;
                        // border: 1/@rem * 1rem solid #4E608E;
