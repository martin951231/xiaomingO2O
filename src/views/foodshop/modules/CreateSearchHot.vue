  ...
  <a-modal :title="title" :width="640" :visible="visible" :confirmLoading="confirmLoading" @ok="handleSubmit" @cancel="handleCancel">
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="是否热门" :labelCol="labelCol" :wrapperCol="wrapperCol" help="选择热门在页面展示会有高亮的标识">
          <a-switch checked-children="是" un-checked-children="否"  v-decorator="['is_hot',{initialValue:detail.is_hot==1 ? true : false,valuePropName: 'checked'}]" />
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
    ...
    methods: {
      ...
      handleSubmit() {
        ...
        validateFields((errors, values) => {
          if (!errors) {
            values.id = this.id
            this.request(foodshopPlatformApi.saveSearchHot, values)
              .then((res) => {
                if (this.id>0) {
      ...
      getEditInfo() {
        this.request(foodshopPlatformApi.getSearchHotDetail, {
            id: this.id
          })
          .then((res) => {
              this.detail = res
