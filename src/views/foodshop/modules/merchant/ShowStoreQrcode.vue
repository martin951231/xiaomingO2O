...
<template>
  <a-modal :title="title" :width="720" :visible="visible" :footer="null" @cancel="handleCancel">
    <div class="content">
      <div class="code-box">
        <div class="code">
          <img src="../../../../assets/images/store_qr.png" />
          <div class="title_code">
            <b>{{ L('已装修的通用码(示例)') }}</b>
          </div>
        </div>
        <div class="qr_code">
          <img src="../../../../assets/images/store_normal_qr.png" />
      ...
      <div class="footer-box">
        <a-button type="primary" icon="download" @click="storeQrcode">{{ L('点击下载') }}</a-button>
...
export default {
  ...
  methods: {
    ...
    storeQrcode() {
      this.title = this.L('选择您需要下载的通用码类型')
      // this.initForm();
      this.request(foodshopMerchantApi.downloadQrcodeStore, {
        store_id: this.storeId,
        is_common: this.radioValue,
      }).then((res) => {
        this.url = res.download_url
        this.img_name = res.img_name
        download(this.url, this.img_name)
      })
    },
    getCode() {
...
function download(url, img_name) {
