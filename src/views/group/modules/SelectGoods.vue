...
<template>
  ...
  >
    <div class="select-goods">
      <div class="left scrollbar" v-if="isLeftMenu">
        <a-menu
          mode="inline"
          :open-keys="openKeys"
      ...
      <div class="right">
        <div class="top">
          <span class="tips" v-if="selectType == 1">同一商家只可选择一个商品参与组合活动</span>
          <span class="tips" v-else></span>

          <!-- 搜索栏 -->
          <a-input-search
            placeholder="商品名称/商家名称/商品id"
            class="search"
            v-model="keywords"
            @search="onSearch"
...
export default {
  ...
  props: {
    ...
    // 1：团购组合 同一商家只可选择一个商品参与组合活动 2：团购装修 同一商家可以添加多个商品
    selectType: {
      type: Number,
      default: 1,
    },
  ...
  methods: {
    ...
    handleList() {
      ...
      if (this.sList.length) {
        this.sList.forEach((item) => {
          this.selectedRowKeys.push(item.group_id)
          this.merIdArr.push(item.mer_id)
        })
      }
      if (this.list.length) {
        this.list.forEach((item, i) => {
          if (this.selectedRowKeys.indexOf(item.group_id) != -1) {
    ...
    onRowSelect(record, selected, selectedRows) {
      ...
      if (selected) {
        ...
        if (record.flag == 'group_renovation') {// 团购装修 商品可随意添加 没有限制
          this.sList.push(record)
          this.selectedRowKeys.push(record.group_id)
          this.merIdArr.push(record.mer_id)
        } else {
          // 验证商家id
          if (this.merIdArr.indexOf(record.mer_id) != -1 && this.isMerIdSelect) {
        ...
        if (this.selectedRowKeys.indexOf(record.group_id) != -1) {
          this.merIdArr.remove(record.mer_id)
          this.sList.remove(record)
          this.selectedRowKeys.remove(record.group_id)
        }
    ...
    onSelectAll(selected, selectedRows, changeRows) {
      ...
      if (selected) {
        ...
        changeRows.map((item) => {
          ...
          this.selectedRowKeys.remove(item.group_id)
