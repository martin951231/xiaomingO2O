    ...
    <div id="components-layout-demo-basic">
        <a-layout>
            <a-row style="width: 100%;height:68px;border-bottom: 1px solid #f1f1f1;background-color: white;line-height: 68px;padding-left: 10px;">
                <a-col :span="24">
                    <a-col :span="3" class="text-right" style="padding-right: 20px;">
                        <a-cascader
                    ...
                    <a-col :span="2" class="text-right" style="padding-right: 20px;">
                        <a-select style="width: 120px" placeholder="发布人" @change="handleHrChange">
                            <a-select-option v-for="item in searchForm.hrlist" :key="item.hr_id">
                                {{ item.hr_name }}
                            </a-select-option>
                        </a-select>
                    </a-col>
                    ...
                    <a-col :span="2">
                        <a-button type="primary" @click="getFindJobList">查询</a-button>
                    </a-col>
                </a-col>
            </a-row>
            <a-layout style="padding: 0 20px; background: #fff">
                <a-layout-content :style="{ margin: '0px', padding: '0px', background: '#fff', minHeight: '100px' }">
                    <a-table :columns="columns"
                        ...
                        <router-link
                                :to="{ path: '/recruit/merchant.recruit/TalentManagementList', query: { job_id: record.job_id } }"
                                slot="deliveries_nums"
                                slot-scope="text, record"
                        >
                            <span v-if="text == 0" >无</span>
                            <a class="label-sm-1 blue" v-else>{{text}}</a>
                        </router-link>

                        <span slot="deliveries_nums" slot-scope="text, record">
                            <span v-if="text == 0" >无</span>
                            <router-link
                                :to="{ path: '/recruit/merchant.recruit/TalentManagementList', query: { job_id: record.job_id } }"
                                v-else
                        ...
                        <span slot="action" slot-scope="text, record">
                            <a v-if="record.status == 1" target="_blank" @click="onlineIt(record.job_id,0)">
                                  下线
                            </a>
                            <a v-else @click="onlineIt(record.job_id,1)">
                                上线
                            </a>
...
<script>
    ...
    import JobDetail from "../merchant/modules/jobDetail";
    import TalentManagementList from "./TalentManagementList";
    ...
    export default {
        name: "MerchantJobList",
        components: {
            TalentManagementList,
            ACol,
            ARow,
            JobDetail,
        },
        ...
        watch: {
            '$route.query.author'(val) {
                if (val) {
                    this.queryParam.author = val
                    this.getJobList()
                } else {
                    this.queryParam.author = 0
                }
            },
        },
        mounted() {
            this.queryParam.author = this.$route.query.author > 0 ? this.$route.query.author : 0
            this.getJobSearch()
            this.getJobList()
        },
        methods: {
            getJobSearch() {
                this.request(recruitMerchantApi.getJobSearch).then((res) => {
            ...
            getJobList() {
                this.request(recruitMerchantApi.getJobList, this.queryParam).then((res) => {
                    this.data = res.list
            ...
            onlineIt(job_id, val) {
                this.$confirm({
                    ...
                    centered: true,
                    onOk: () => {
                        this.request(recruitMerchantApi.updateJob, {
                            status: val,
                            job_id: job_id
                        }).then((res) => {
            ...
            delIt(job_id) {
                this.$confirm({
                    ...
                    centered: true,
                    onOk: () => {
                        this.request(recruitMerchantApi.delJob, {
                            job_id: job_id
                        }).then((res) => {
                            this.queryParam = {
                                page: 1,
