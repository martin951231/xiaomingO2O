    ...
    <div class="bg-ff wrap">
        ...
        <div class="content">
            ...
            <a-form-model :model="searchForm" layout="inline" :labelCol="{ span: 5 }" :wrapperCol="{ span: 19 }">
                ...
                <a-row type="flex">
                    <a-col :span="10">
                        <a-form-model-item label="物业名">
                            <a-input v-model="searchForm.property_name" placeholder="请输入物业名称"> </a-input>
                        </a-form-model-item>
                    </a-col>
                    <a-col :span="10">
            ...
            <a-table
                ...
                class="mt-20"
            >
                <span slot="show_order" slot-scope="text, record">
        ...
        >
            <a-form-model :model="modalForm" :label-col="{ span: 6 }" :wrapper-col="{ span: 14 }">
                <a-form-model-item
                    label="选择接手成员"
                    help="转移后此物业后续业务归接手成员服务，且业务提成均属接手成员"
                >
                    <a-cascader
...
export default {
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    ...
                    scopedSlots: { customRender: 'show_order' },
                    align: 'center',
                },
                {
                    title: '注册时间',
    ...
    methods: {
        ...
        getList(update = false) {
            ...
            this.request(marketingPlatformApi.getHousePropertyList, params).then((res) => {
                this.list = res.list || []
        ...
        // storeDetail = 已创建店铺 businessTransfer = 业务转移 detail= 物业信息订单查看
        actionBtn(current, type) {
            if (type == 'detail') {
                // 跳转详情页
                this.$router.push({ path: '/new_marketing/platform/HousePropertyDetail', query: { id: current.property_id } })
