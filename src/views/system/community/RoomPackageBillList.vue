...
const columns = [
  { title: '套餐标题',  dataIndex: 'details_info.room_title', key: 'room_title'},
...
const package_columns = [
  { title: '套餐标题',  dataIndex: 'room_title', key: 'room_title_detail'},
...
export default {
  ...
  mounted () {
    this.getRoomPackageBillList()
  },
  methods: {
    // 获取列表信息
    getRoomPackageBillList() {
      this.search['page'] = this.page;
      let _this = this
      this.request(packageApi.packageRoomOrderList, this.search)
        .then((res) => {
          console.log('res',res)
    ...
    // 获取详情
    getPackageRoomOrderInfo(order_id) {
      let _this = this
      this.request(packageApi.packageRoomOrderInfo, { order_id: order_id})
        .then((res) => {
          console.log('res',res)
          _this.detail = res
    ...
    look(e) {
      console.log('e', e)
      this.visible = true
      this.getPackageRoomOrderInfo(e.order_id)
    },
    // 表格中变动  这里只针对页面页数切换
    table_change(e) {
      console.log('e', e)
