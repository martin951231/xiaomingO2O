...
<template>
    <div class="scroll_content" id="scroll_content">
        <div v-if="showCharts" id="charts_container_4" :style="{height: heightItem * 45 + 'px'}"></div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            getPartyActivity(){
                let that = this
                let postArr=this.addressParams;
                postArr.page=this.pageInfo.page
                postArr.limit=this.pageInfo.limit
                that.request(houseCityApi.getStreetPartyActivity,postArr).then((res) => {
                    that.count = res.count
                    that.limit = res.total_limit
                    that.url = res.url
                    that.computeMaxpage(res.count)
                    that.list = [...that.list, ...res.list]
                    that.heightItem = that.list.length
                    let data = []
                    let percentdata = []
                    let leftname = []
                    that.list.map((v, i)=>{
                        data.push(v.count)
                        percentdata.push(v.ratio)
                        leftname.push(v.area_name)
                    })
                    ...
                    that.$nextTick(()=>{
                        that.workingplace(params)
                    })
                })
            },
            workingplace(msg) {
                console.log("msg===>", msg)
                this.heightItem = this.list.length
                this.showCharts = false
                let that = this
                console.log("this.heightItem===>", this.heightItem)
                var totaldata = [];
                var percentdata = [];
                msg.data.map(v=>{
                    totaldata.push(100)
                ...
                myCharts = echarts.init(document.getElementById('charts_container_4'));
            ...
            bindScroll(ref) {
                let that = this;
                let content = document.getElementById('scroll_content');
                content.addEventListener('scroll', function() {
                    if(content.scrollTop + content.clientHeight == content.scrollHeight){
                        console.log("触底了。。。")
                        if(that.list.length>0){
                        	if(that.pageInfo.page>=that.maxPage){
                        		that.noMore = true
                        	} else{
            ...
            computeMaxpage(count){
            	this.noMore = false
            	if(count % this.pageInfo.limit == 0){
            		this.maxPage = parseInt(count / this.pageInfo.limit)
            	} else{
            		this.maxPage = parseInt(count / this.pageInfo.limit + 1)
            	}
            },
        }
    }
