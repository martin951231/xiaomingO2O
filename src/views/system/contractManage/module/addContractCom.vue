        ...
        <a-form-model ref="ruleForm" :model="contractForm" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol" v-if="refrashForm">
            ...
            <a-form-model-item label="工作人员" prop="work_id">
                <a-select v-model="contractForm.work_id" placeholder="请选择" @change="(value)=>handleSelectChange(value, 'work_id')">
                    <a-select-option :value="item.wid" v-for="(item, index) in workList" :key="index">
                        {{item.name}}
                    </a-select-option>
                </a-select>
            </a-form-model-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onConfirm() {
                this.$refs.ruleForm.validate(valid => {
                    if (valid) {
                        ...
                        params.village_id = this.contractForm['village_id'].split('-')[1]
                        if(this.contractId){
                            params.id = this.contractId
                        }
                        this.request('/community/platform.contract.Index/setContractInfo', params).then(res=>{
                            if(this.contractId){
            ...
            getWorkList(id){
                this.request('/community/platform.RepairWorkOrder/getVillageWorkList', {village_id: id}).then(res=>{
            ...
            getDetail(id){
                this.request('/community/platform.contract.Index/getContractInfo', {id, }).then(res=>{
            ...
            getformConfig(){
                this.request('/community/platform.contract.Index/getAddUpdateContractConfig', {}).then(res=> {
