<template>
  <div class="mt-10 mb-20 pt-20 pl-20 pr-20 pb-20 bg-ff br-10">
    <div>
      <a-tabs default-active-key="1"><a-tab-pane tab="分类管理"></a-tab-pane></a-tabs>
    </div>
    <div>
      <a-form-model-item>
        <a-button type="primary" @click="$refs.createModal.add()" class="ml-20 maxbox"> 添加分类</a-button>
        <a-button @click="delAll()" class="ml-20 maxbox"> 删除</a-button>
      </a-form-model-item>
    </div>
    <!-- 数据列表 -->
    <a-table class="mt-20" rowKey="cat_id" :columns="columns" :data-source="dataList" :row-selection="rowSelection" :pagination="pagination">
      <span slot="sort" slot-scope="text, record">
          <a-input-number :min="0" step="1" style="width: 100px" v-model="record.sort"
          @blur="handleSortChange(text,record.cat_id)"/>
      </span>
      <!-- 操作栏 -->
      <span slot="action" slot-scope="text">
       <a class="ml-10 inline-block" @click="$refs.createModal.edit(text)">编辑</a>
       <a class="ml-10 inline-block" @click="delAll(text)">删除</a>
      </span>
    </a-table>
    <category-create @loaddata="getDataList" ref="createModal"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getDataList(param) {
      ...
      this.request(marriageHelperCommentApi.getCategoryList, params).then((res) => {
    ...
    delAll(id) {
      ...
      const modal = this.$confirm({
        ...
        centered: true,
        onOk: () => {
          this.request(marriageHelperCommentApi.getCategoryDelAll, { ids }).then((res) => {
