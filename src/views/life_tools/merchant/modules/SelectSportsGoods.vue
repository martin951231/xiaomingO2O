          ...
          <template>
            ...
            <a-sub-menu key="体育课程">
              <span slot="title">
                ...
                <!-- 一级菜单（但是有多级菜单） -->
                <template v-for="menu of sortListCourse">
                  <a-sub-menu v-if="menu.children && menu.children.length" :key="menu.id">
                    <span slot="title">
                      <span>{{ menu.name }}</span>
                    </span>
...
export default {
  ...
  watch: {
    dialogVisible (val) {
      if (val) {
        ...
        } else if (this.source == 'limitedSport') {
          this.titleName = '选择体育门票'
        }
      }
    },
    selectedList (val) {
  ...
  methods: {
    ...
    // 获取左侧分列列表
    getSortList() {
      this.request(mallMerchantApi.getMerchantSort,{source:'stadium'}).then((res) => {
        if (res.list && res.list.length) {
          this.sortList = res.list
          this.handleDefaultSelect()
        }
      })
      this.request(mallMerchantApi.getMerchantSort,{source:'course'}).then((res) => {
        if (res.list && res.list.length) {
          this.sortListCourse = res.list
        }
      })
    },
