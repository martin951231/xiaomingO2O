...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 新建活动
            createActivity() {
                this.$router.push({ path: '/merchant/merchant.life_tools/scenicToolsLimitEdit', query: {} })
            },
            // 编辑活动
            editActivity(id, status) {
                if (status == 2) {
                    this.$router.push({ path: '/merchant/merchant.life_tools/scenicToolsLimitLook', query: { store_id: this.store_id, id } })
                } else {
                    this.$router.push({ path: '/merchant/merchant.life_tools/scenicToolsLimitEdit', query: { store_id: this.store_id, id } })
                }
            },
            // 失效
