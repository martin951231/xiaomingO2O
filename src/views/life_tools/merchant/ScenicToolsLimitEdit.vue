                        ...
                        <span slot="actStockNum" slot-scope="text,record">
                          <a-form-item style="margin: -5px 0" prop="act_stock_num">
                              <a-input-number
                                      ...
                                      @change="handleChange(record.act_stock_num, record, 'act_stock_num',record.ticket_id)"
                              />
                          </a-form-item>
                  </span>
            ...
            batchSet(e, type) {
                ...
                this.goodsList = this.goodsList.map(item => {
                    if (type == 'reduce_money') {//减少金额,计算折扣比例
                        ...
                        item.discount_rate = 10 - discount_rate
                    } else {//折扣比例,计算减少金额
                    ...
                    if (valid) {
                        ...
                        let actStockNumCheck = JSON.parse(JSON.stringify(this.goodsList)).filter(item => {
                            /*if (!item.sku_info || (item.sku_info && !item.sku_info.length)) {*/
                                if (item.act_stock_num && Number(item.act_stock_num) > Number(item.stock_num) && item.stock_num != -1 && item.stock_type == 1) {
                                    return item
                                }
                        ...
                        this.request(mallMerchantApi.updateLimited, formData).then((res) => {
                            ...
                            this.$router.push({path: '/merchant/merchant.life_tools/scenicToolsLimitList'})
