    ...
    <div class="mt-10 mb-20 mh-full">
        ...
        >
            ...
            <a-card style="margin-top: 10px" title="优惠信息" :bordered="false">
                ...
                <a-form-model-item label="选择活动库存类型" prop="stock_type">
                    <a-radio-group v-model="formData.stock_type">
                        <a-radio :value="1"> 活动总库存</a-radio>
                        <a-radio :value="2"> 活动每日库存</a-radio>
                    </a-radio-group>
                </a-form-model-item>
            </a-card>

            <a-card style="margin-top: 10px" title="商品信息" :bordered="false">
                <a-form-model-item label="活动门票" prop="goods_info">
                    <a-button @click="addProduct()" type="primary"> 添加门票 </a-button>
                ...
                <a-table
                    ...
                    <span slot="day_stock_num" slot-scope="text,record">
                        <a-form-item style="margin: -5px 0" prop="day_stock_num">
                            <a-input-number
                                style="width:50px"
                                :min="0"
                                :max="record.stock_num - 0"
                                v-model="record.day_stock_num"
                                @change="handleChange(record.day_stock_num, record, 'day_stock_num',record.ticket_id)"
                              />/
                              <a-input-number
                                style="width:50px"
                                :min="0"
                                :max="record.stock_num - 0"
                                v-model="record.day_original_stock_num"
                                @change="handleChange(record.day_original_stock_num, record, 'day_original_stock_num',record.ticket_id)"
                            />
                        </a-form-item>
                    </span>
                    <span slot="actStockNum" slot-scope="text,record">
                        <a-form-item style="margin: -5px 0" prop="act_stock_num">
                            <a-input-number
                                ...
                                @pressEnter="handleChange(record.act_stock_num, record, 'act_stock_num',record.ticket_id)"
                            />
                        </a-form-item>
                    </span>
                    <span slot="price" slot-scope="text" class="cr-red">
                        ￥{{ text }}
                    </span>
                    <span slot="reduceMoneyTitle">
                        减少金额
                        <a-tooltip trigger="hover">
                            <template slot="title">设置每张门票的减少金额</template>
                            <a-icon class="ml-10" type="question-circle" />
                        </a-tooltip>
                    </span>
                    <span slot="reduceMoney" slot-scope="text,record">
                        <a-form-item style="margin: -5px 0">
                            <a-input-number
                                ...
                                @change="handleChange(record.reduce_money, record, 'reduce_money',record.ticket_id)"
                            />
                    </a-form-item>
                  </span>
                    <span slot="discountRateTitle">
                        折扣比例
                        <a-tooltip trigger="hover">
                            <template slot="title">设置每张门票的折扣比列（范围0-10）</template>
                            <a-icon class="ml-10" type="question-circle" />
                        </a-tooltip>
                    </span>
                    <span slot="discountRate" slot-scope="text,record">
                        <a-form-item style="margin: -5px 0">
                            <a-input-number
                                ...
                                @change="handleChange(record.discount_rate, record, 'discount_rate',record.ticket_id)"
                            />
                        </a-form-item>
                    </span>
...
<script>
    ...
    import SelectSportsGoods from './modules/SelectSportsGoods'
    import { watchMixin } from '@/utils/mixin'
    export default {
        name: "ScenicToolsLimitEdit",
        mixins: [watchMixin],
        components: {SelectSportsGoods},
        data() {
            return {
                id: '', // 活动id
                ...
                columns: [
                    ...
                    {
                    title: '活动当日库存/活动每日库存',
                    dataIndex: 'day_stock_num',
                    scopedSlots: { customRender: 'day_stock_num' },
                    },
                    {
                        title: '原价格',
                        dataIndex: 'price',
        ...
        methods: {
            ...
            // 表单信息
            getFormData() {
                this.request(mallMerchantApi.getSportsSecondsKillDetail, {id: this.id}).then((res) => {
            ...
            //活动库存  减少金额  折扣比例设置
            handleChange(text,record,type,ticket_id) {
                this.goodsList = this.goodsList.map(item => {
                    if(item.ticket_id==ticket_id){
                        item[type] = this[type]
                        if (type == 'act_stock_num') {
                            item.act_stock_num = text
                        } else if (type == 'day_stock_num') {//每日库存
                            item.day_stock_num = text
                        } else if (type == 'day_original_stock_num') {//每日库存的原始库存
                            item.day_original_stock_num = text
                        } else if (type == 'reduce_money') {//减少金额,计算折扣比例
                            item.act_price = Math.round((item.price - text + Number.EPSILON) * 100) / 100
                            item.reduce_money = Math.round((text + Number.EPSILON) * 100) / 100
                            let discount = ((1-text / item.price) * 10).toFixed(2)
                            discount = discount > 0 ? discount : 0.01
                            let discount_rate = item.price > 0 ?  discount  : 0
                            item.discount_rate = discount_rate
                        } else if (type == 'discount_rate') {//折扣比例,计算减少金额
                            item.discount_rate = text
                            let reduce_money = (1 - text / 10) * item.price
                            item.reduce_money = Math.round((reduce_money + Number.EPSILON) * 100) / 100
                            item.act_price = Math.round((item.price - item.reduce_money + Number.EPSILON) * 100) / 100
                        }
                    }
                    return item
                })
            ...
            batchSet(e, type) {
                ...
                this.goodsList = this.goodsList.map(item => {
                    if (type == 'reduce_money') {//减少金额,计算折扣比例
                        item.act_price = Math.round((item.price - this.reduce_money + Number.EPSILON) * 100) / 100
                        item.reduce_money = Math.round((this.reduce_money + Number.EPSILON) * 100) / 100
                        let discount_rate = item.price > 0 ? ((this.reduce_money / item.price) * 10).toFixed(2) : 0
                        item.discount_rate = (10 - discount_rate).toFixed(2)
                    } else {//折扣比例,计算减少金额
                        item.discount_rate = this.discount_rate
                        let reduce_money = (1 - item.discount_rate/10) * item.price
                        item.reduce_money = Math.round((reduce_money + Number.EPSILON) * 100) / 100
                        item.act_price = Math.round((item.price - item.reduce_money + Number.EPSILON) * 100) / 100
                    }
                    return item
            ...
            addProduct() {
                ...
                this.$refs.selectSportsGoods.openDialog()
            },
            // 选择商品回调
            selecrGoodsSubmit(e) {
                ...
                e.goods = e.goods.map((item) => {
                    ...
                    item.act_stock_num=item.act_stock_num
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        if (this.goodsList.length) {
                            formData.goods_sku = this.goodsList
                        }else {
                            this.$message.error('请选择活动商品')
                            return
                        }
                        // 活动库存校验
                        let StockNumCheck = this.goodsList.filter(item => {
                            console.log(item.act_stock_num,'item.act_stock_num')
                                if(item.act_stock_num == null){
                                    return item
                                }
                        }) || []
                        if (this.formData.stock_type == 1) {
                            if (StockNumCheck.length) {
                                this.$message.error('请填写活动总库存')
                                return
                            }
                        }
                        ...
                        // 活动库存校验
                        let actStockNumCheck = this.goodsList.filter(item => {
                                if (item.stock_type == 1 && item.act_stock_num && Number(item.act_stock_num) > Number(item.stock_num) && item.stock_num != -1) {
                                    return item
                                }
                        }) || []
                        ...
                        if(formData.stock_type == 2){// 每日库存
                            // let dayStockNumCheck = this.goodsList.filter(item => {
                            //         if (item.day_stock_num && Number(item.day_stock_num) > Number(item.act_stock_num) ) {
                            //             return item
                            //         }
                            // }) || []
                            // if (dayStockNumCheck.length) {
                            //     this.$message.error('商品活动当前库存不能大于活动总库存')
                            //     return
                            // }

                            // 活动库存校验
                            let dayOriginalStockNumCheck = this.goodsList.filter(item => {
                                    if (item.day_stock_num && Number(item.day_stock_num) > Number(item.day_original_stock_num) ) {
                                        return item
                                    }
                            }) || []
                            if (dayOriginalStockNumCheck.length) {
                                this.$message.error('商品活动当前库存不能大于活动每日原始库存')
                                return
                            }
                        }
                        ...
                        this.request(mallMerchantApi.saveSportsSecondsKill, formData).then((res) => {
                            this.resetForm()
                            if (this.id) {
                                this.getFormData()
                            }
                            this.$message.success('提交成功！')
                            this.$router.push({path: '/merchant/merchant.life_tools/SportsSecondsKillActList'})
