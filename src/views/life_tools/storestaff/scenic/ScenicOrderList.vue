  ...
  <div style="margin-top: 20px;padding:20px;background-color: #fff;">
    ...
    <a-table :row-selection="rowSelection" rowKey="order_id" :columns="columns" :data-source="datalist" :pagination="pagination" bordered>
      ...
      <span slot="action" slot-scope="text, record">
        ...
        <a v-print="'#printer'" @click.stop="onconfirmCode(record)" style="margin-left: 10px;">打印</a>
      </span>
    </a-table>
    ...
    <div class="main_body" id="printer">
      <div style="margin-bottom: 5px;" v-for="(item, index) in printDetail.print_data" :key="index">
        <p style="margin-bottom: 10px; font-size: 14px; margin-left:15px;">日期：{{ printDetail.ticket_time }}</p>
...
export default {
  ...
  data() {
    return {
      ...
      searchForm: {
        ...
        exportUrl: lifeToolsStorestaffApi.exportToolsOrder,
        selectedRowKeys: [],
        selectedRows: [],
  ...
  methods: {
    ...
    getDataList(is_search) {
      ...
      this.request(lifeToolsStorestaffApi.getScenicOrderList, params).then((res) => {
    ...
    handleDetail(order_id) {
      this.request(lifeToolsStorestaffApi.getScenicOrderDetail, {order_id: order_id}, 'GET').then((res) => {
        this.currentBtn = {
          'props': 'orderDetail',
          'title': '订单详情',
          'data': res
    ...
    onDateRangeChange(date, dateString) {
      ...
      agreeAll() {
          ...
          this.request(lifeToolsStorestaffApi.agreeScenicOrderRefund, {order_ids: ids}).then((res) => {
      ...
      refuseOk() {
          ...
          this.request(lifeToolsStorestaffApi.refuseScenicOrderRefund, {order_ids: ids, reason: reason}).then((res) => {
              this.$message.success('操作成功')
              this.refusevisible = false
              this.selectedRowKeys = []
              this.selectedRows = []
