<template>
  <a-modal :bodyStyle="{'height':'75vh','overflow-y': 'auto'}" :width="'70%'" title="热门推荐" :visible="dialogVisible" @cancel="handleCancel" @ok="handleOk">
    <div>
            ...
            <a-form :form="form" :label-col="{ span: 3 }" :wrapper-col="{ span: 17 }">
              ...
              <a-form-item label="是否展示">
                <a-switch 
                  ...
                  :checked="configData['life_tools_sports_index_'+item.type+'_status'] == 1 ? true : false"
                  @change="changeValue($event, 'life_tools_sports_index_'+item.type+'_status')"
                />
              </a-form-item>
              <a-form-item label="主标题">
                <a-input 
                  v-model="configData['life_tools_sports_index_'+item.type+'_desc']"
                  style="width: 300px;"
                  placeholder="请输入主标题" 
                  @change="changeValue($event, 'life_tools_sports_index_'+item.type+'_desc')"
                  />
              </a-form-item>
              <a-form-item label="排序">
                <a-input-number
                  :min="0" 
                  v-model="configData['life_tools_sports_index_'+item.type+'_sort']"
                  style="width: 300px;"
                  placeholder="请输入排序值" 
                  @change="changeValue($event, 'life_tools_sports_index_'+item.type+'_sort')"
                  />
              </a-form-item>
              <a-form-item label="更多跳转链接">
                <a-input
                  :min="0" 
                  v-model="configData['life_tools_sports_index_'+item.type+'_url']"
                  style="width: 300px;"
                  placeholder="请输入更多跳转链接" 
                  @change="changeValue($event, 'life_tools_sports_index_'+item.type+'_url')"
                  />
                  <a class="ant-form-text" @click="setLinkBases('life_tools_sports_index_'+item.type+'_url')">
                    从功能库选择
                  </a>
              </a-form-item>
      ...
      <select-sports-hot-recommend
        ref="SelectSportsHotRecommend"
...
<script>
  ...
  import SelectSportsHotRecommend from "../modules/SelectSportsHotRecommend";
  ...
  export default {
    ...
    components: {SelectSportsHotRecommend,LinkBases},
    data() {
      return {
        data: [],
        columns : [],
        ...
        configData:{
          life_tools_sports_index_stadium_status:'0',
          life_tools_sports_index_course_status:'0',
          life_tools_sports_index_competition_status:'0',
          life_tools_sports_index_mall_status:'0',
        },
    ...
    methods: {
      openDialog() {
        ...
        if(this.type == 'competition'){
          this.columns = this.competitionColumns
        }else{
      ...
      getTabs() {
        this.request(lifeToolsPlatformApi.getSportsIndexHotRecommond).then(res => {
          this.tabArr = res.tab_arr
          this.configData = res.config_list
        })
      },
      getList() {
        this.request(lifeToolsPlatformApi.getSportsHotRecommendSelectedList, {page: this.page,type:this.type}).then(res => {
          this.data = res.data;
      ...
      getGoods() {
        ...
        this.$refs.SelectSportsHotRecommend.openDialog(this.type, this.typeName)
      ...
      handleSortChange(e, val, record) {
        ...
        this.request(lifeToolsPlatformApi.saveSportsHotRecommendSort, params).then((data) => {
          this.getList()
        })
      },
      //删除关联商品
      delOne(id) {
        ...
        this.request(lifeToolsPlatformApi.delSportsHotRecommendList, params).then((data) => {
      ...
      delAll(){
        ...
        this.$confirm({
          ...
          okType: 'danger',
          onOk() { 
            _this.request(lifeToolsPlatformApi.delSportsHotRecommendList, params).then((data) => {
