  ...
  <div style="margin-top: 5px;padding:10px;background-color: #fff;">
    <a-form-model layout="inline" :model="searchForm">
      <a-form-model-item label="公告标题">
        <a-input v-model="searchForm.title" placeholder="公告标题"></a-input>
      </a-form-model-item>
        <a-form-model-item>
  ...
  methods: {
    ...
    getDataList(is_search) {
      ...
      params.pageSize = this.pagination.pageSize
        params.type = this.life_type
      this.request(lifeToolsPlatformApi.getInfoList, params).then((res) => {
    ...
    handleOk() {
        if (this.selectedRowKeys.length) {
          this.request(lifeToolsPlatformApi.addRelatedInfo, {
            information_id: this.selectedRowKeys,
          }).then(res => {
