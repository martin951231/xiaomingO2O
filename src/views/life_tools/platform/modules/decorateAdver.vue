<template>
  <a-modal :title="title" :width="1100" :height="600" :visible="visible" @cancel="handelCancle" @ok="handleOk" footer="">
    <div>
      <a-button v-if="this.cat_key != 'wap_life_tools_ticket_slider'" type="primary" style="margin-bottom: 10px" @click="getAddModel">新建</a-button>
      <a-table :columns="columns" rowKey="id" :data-source="list" :scroll="{ y: 700 }">
        ...
        <span slot="action" slot-scope="text, record">
              ...
              <a-divider v-if="record.cat_id != 6" type="vertical"/>
              <a-popconfirm v-if="record.cat_id != 6" title="确认删除？" ok-text="确定" cancel-text="取消" @confirm="delOne(record.id)">
                 <a>删除</a>
              </a-popconfirm>
            </span>
      </a-table>
      <a-modal :visible="add_visible" width="650px" @cancel="handleCancle" :closable="false" @ok="handleSubmit" :confirmLoading="confirmLoading" :destroyOnClose="true">
        <div style="overflow-y:scroll; height: 600px">
          ...
          >
            ...
            <a-form-item label="选择苹果APP" v-if="app_open_type == 2">
            ...
            <a-form-item label="苹果APP下载地址" v-if="app_open_type == 2">
              <a-input
                placeholder="请输入苹果APP下载地址"
                v-decorator="['ios_app_url']"/>
            </a-form-item>
            <a-form-item label="安卓APP包名" v-if="app_open_type == 2">
              <a-input
                placeholder="请输入安卓APP包名"
                v-decorator="['android_app_name']"/>
            </a-form-item>
            <a-form-item label="安卓APP下载地址" v-if="app_open_type == 2">
              <a-input placeholder="请输入安卓APP下载地址"
                       v-decorator="['android_app_url']"/>
            </a-form-item>
            <a-form-item label="打开其他小程序" v-if="app_open_type == 1">
            ...
            <a-form-item label="小程序页面" v-if="app_open_type == 1">
              <a-input
                placeholder="请输入小程序页面路径"
...
<script>
  ...
  import DecorateAdverEdit from "@/views/life_tools/platform/modules/decorateAdverEdit";
      ...
      this.getAllArea()
        if (this.cat_key == 'wap_life_tools_ticket_slider') {
            this.columns = this.columns.filter(item => {
                return item.dataIndex != 'area_name'
            })
            this.columns = this.columns.filter(item => {
                return item.dataIndex != 'pic'
            })
        }
    },
    methods: {
      ...
      getList(cat_key, titles) {
        ...
        this.request(lifeToolsPlatformApi.getList, {cat_key: cat_key}).then(res => {
          console.log(res)
      ...
      delOne(id) {
        this.request(lifeToolsPlatformApi.getDel, {id: id}).then(res => {
      ...
      editOne(e) {
        ...
        this.tab_key = e
        if (e == 2) {
          this.request(lifeToolsPlatformApi.getEdit).then(res => {
