    ...
    <div class="mt-10 mb-20 mh-full">
        ...
        >
            <a-card  :bordered="false">
                ...
                <a-form-model-item label="每行显示几列" v-if="recommend.goods_type == 'mall'">
                    <a-select v-model="recommend.column_num" @change="updateStatus(recommend.id)">
                        <a-select-option v-for="item in columnNumArr" :value="item.value" :key="item.value">{{item.name}}</a-select-option>
                    </a-select>
                </a-form-model-item>
            </a-card>
            <a-card style="margin-top: 10px" title="商城商品信息" :bordered="false">
                <a-form-model-item label="商品" :wrapper-col="{span: 23}" :labelCol="{ span: 1}">
                    <a-row>
                        <a-col :span="12" class="text-left">
                            <a-button @click="addProduct('mall')" type="primary"> 添加商品 </a-button>
                        </a-col>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getOpen() {
                this.request(lifeToolsPlatformApi.getRecGoodsList, {goods_type:'mall'}).then((res) => {
            ...
            updateStatus(id) {
                this.request(lifeToolsPlatformApi.updateRec, {
                    ...
                    title: this.recommend.title,
                    sort: this.recommend.sort,
                    column_num: this.recommend.column_num,
                    goods_type: this.recommend.goods_type
                }).then((res) => {
