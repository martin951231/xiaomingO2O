<template>
  ...
  <div style="margin-top: 5px;padding:10px;background-color: #fff;">
    <a-form-model layout="inline" :model="searchForm">
      <a-form-model-item label="景区标题">
        <a-input v-model="searchForm.title" placeholder="景区标题"></a-input>
      </a-form-model-item>
        <a-form-model-item>
    ...
    >
      <span slot="action" slot-scope="text, record">
             <a @click="handleOk(record.tools_id)">选中</a>
      </span>
    </a-table>
  </div>
  </a-modal>
...
export default {
  ...
  methods: {
    ...
    getDataList(is_search) {
      ...
      this.request(lifeToolsPlatformApi.getScenicList, params).then((res) => {
    ...
    handleOk(tools_id) {
        if (tools_id) {
          this.request(lifeToolsPlatformApi.addRelatedScenic, {
            tools_id: tools_id,
          }).then(res => {
